...
public class BlockCauldron extends Block {
    ...
    public void onEntityCollidedWithBlock(World worldIn, BlockPos pos, IBlockState state, Entity entityIn) {
        ...
        if (!worldIn.isRemote && entityIn.isBurning() && 0 < i && entityIn.getEntityBoundingBox().minY <= (double) f) {
            entityIn.extinguish();
        ...
        if (worldIn.isRemote) {
            ...
            if (null == itemstack) {
                ...
                if (item == Items.water_bucket) {
                    ...
                    if (0 < i && item instanceof ItemArmor) {
                        ItemArmor itemarmor = (ItemArmor) item;

                        if (ItemArmor.ArmorMaterial.LEATHER == itemarmor.getArmorMaterial() && itemarmor.hasColor(itemstack)) {
                    ...
                    if (0 < i && item instanceof ItemBanner && 0 < TileEntityBanner.getPatterns(itemstack)) {
                        ItemStack itemstack1 = itemstack.copy();
                        itemstack1.stackSize = 1;
                        TileEntityBanner.removeBannerData(itemstack1);

                        if (1 >= itemstack.stackSize && !playerIn.capabilities.isCreativeMode) {
    ...
    public void fillWithRain(World worldIn, BlockPos pos) {
        if (1 == worldIn.rand.nextInt(20)) {
            IBlockState iblockstate = worldIn.getBlockState(pos);

            if (3 > iblockstate.getValue(BlockCauldron.LEVEL).intValue()) {
