...
public class BlockDoublePlant extends BlockBush implements IGrowable {
    ...
    public boolean isReplaceable(World worldIn, BlockPos pos) {
        ...
        if (iblockstate.getBlock() != this) {
            ...
            return EnumPlantType.FERN == blockdoubleplant$enumplanttype || EnumPlantType.GRASS == blockdoubleplant$enumplanttype;
        }
    }

    protected void checkAndDropBlock(World worldIn, BlockPos pos, IBlockState state) {
        if (!this.canBlockStay(worldIn, pos, state)) {
            boolean flag = EnumBlockHalf.UPPER == state.getValue(BlockDoublePlant.HALF);
    ...
    public boolean canBlockStay(World worldIn, BlockPos pos, IBlockState state) {
        if (EnumBlockHalf.UPPER == state.getValue(BlockDoublePlant.HALF)) {
    ...
    public Item getItemDropped(IBlockState state, Random rand, int fortune) {
        if (EnumBlockHalf.UPPER == state.getValue(BlockDoublePlant.HALF)) {
            ...
            return EnumPlantType.FERN == blockdoubleplant$enumplanttype ? null : (EnumPlantType.GRASS == blockdoubleplant$enumplanttype ? (0 == rand.nextInt(8) ? Items.wheat_seeds : null) : Item.getItemFromBlock(this));
        }
    }
    ...
    public int damageDropped(IBlockState state) {
        return EnumBlockHalf.UPPER != state.getValue(BlockDoublePlant.HALF) && EnumPlantType.GRASS != state.getValue(BlockDoublePlant.VARIANT) ? state.getValue(VARIANT).getMeta() : 0;
    }

    public int colorMultiplier(IBlockAccess worldIn, BlockPos pos, int renderPass) {
        BlockDoublePlant.EnumPlantType blockdoubleplant$enumplanttype = this.getVariant(worldIn, pos);
        return EnumPlantType.GRASS != blockdoubleplant$enumplanttype && EnumPlantType.FERN != blockdoubleplant$enumplanttype ? 16777215 : BiomeColorHelper.getGrassColorAtPos(worldIn, pos);
    }
    ...
    public void harvestBlock(World worldIn, EntityPlayer player, BlockPos pos, IBlockState state, TileEntity te) {
        if (worldIn.isRemote || null == player.getCurrentEquippedItem() || player.getCurrentEquippedItem().getItem() != Items.shears || EnumBlockHalf.LOWER != state.getValue(BlockDoublePlant.HALF) || !this.onHarvest(worldIn, pos, state, player)) {
        ...
        if (EnumBlockHalf.UPPER == state.getValue(BlockDoublePlant.HALF)) {
            if (worldIn.getBlockState(pos.down()).getBlock() == this) {
                if (!player.capabilities.isCreativeMode) {
                    ...
                    if (EnumPlantType.FERN != blockdoubleplant$enumplanttype && EnumPlantType.GRASS != blockdoubleplant$enumplanttype) {
                        worldIn.destroyBlock(pos.down(), true);
                    } else if (!worldIn.isRemote) {
                        if (null != player.getCurrentEquippedItem() && player.getCurrentEquippedItem().getItem() == Items.shears) {
    ...
    private boolean onHarvest(World worldIn, BlockPos pos, IBlockState state, EntityPlayer player) {
        ...
        if (EnumPlantType.FERN != blockdoubleplant$enumplanttype && EnumPlantType.GRASS != blockdoubleplant$enumplanttype) {
            ...
            int i = (EnumPlantType.GRASS == blockdoubleplant$enumplanttype ? BlockTallGrass.EnumType.GRASS : BlockTallGrass.EnumType.FERN).getMeta();
    ...
    public boolean canGrow(World worldIn, BlockPos pos, IBlockState state, boolean isClient) {
        BlockDoublePlant.EnumPlantType blockdoubleplant$enumplanttype = this.getVariant(worldIn, pos);
        return EnumPlantType.GRASS != blockdoubleplant$enumplanttype && EnumPlantType.FERN != blockdoubleplant$enumplanttype;
    }
    ...
    public IBlockState getStateFromMeta(int meta) {
        return 0 < (meta & 8) ? this.getDefaultState().withProperty(HALF, BlockDoublePlant.EnumBlockHalf.UPPER) : this.getDefaultState().withProperty(HALF, BlockDoublePlant.EnumBlockHalf.LOWER).withProperty(VARIANT, BlockDoublePlant.EnumPlantType.byMetadata(meta & 7));
    }
    ...
    public int getMetaFromState(IBlockState state) {
        return EnumBlockHalf.UPPER == state.getValue(BlockDoublePlant.HALF) ? 8 | state.getValue(FACING).getHorizontalIndex() : state.getValue(VARIANT).getMeta();
    }
    ...
    public enum EnumPlantType implements IStringSerializable {
        ...
        public static BlockDoublePlant.EnumPlantType byMetadata(int meta) {
            if (0 > meta || meta >= META_LOOKUP.length) {
                meta = 0;
            }
