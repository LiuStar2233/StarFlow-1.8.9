...
public class BlockDynamicLiquid extends BlockLiquid {
    ...
    public void updateTick(World worldIn, BlockPos pos, IBlockState state, Random rand) {
        ...
        if (0 < i) {
            ...
            if (2 <= adjacentSourceBlocks && this.blockMaterial == Material.water) {
                IBlockState iblockstate1 = worldIn.getBlockState(pos.down());

                if (iblockstate1.getBlock().getMaterial().isSolid()) {
                    i1 = 0;
                } else if (iblockstate1.getBlock().getMaterial() == this.blockMaterial && 0 == iblockstate1.getValue(BlockLiquid.LEVEL).intValue()) {
                    i1 = 0;
                }
            }

            if (this.blockMaterial == Material.lava && 8 > i && 8 > i1 && i1 > i && 0 != rand.nextInt(4)) {
                k *= 4;
            }
        ...
        } else if (0 <= i && (0 == i || this.isBlocked(worldIn, pos.down(), iblockstate))) {
    ...
    private int func_176374_a(World worldIn, BlockPos pos, int distance, EnumFacing calculateFlowCost) {
        ...
        for (EnumFacing enumfacing : EnumFacing.Plane.HORIZONTAL) {
            if (enumfacing != calculateFlowCost) {
                ...
                if (!this.isBlocked(worldIn, blockpos, iblockstate) && (iblockstate.getBlock().getMaterial() != this.blockMaterial || 0 < iblockstate.getValue(BlockLiquid.LEVEL).intValue())) {
    ...
    protected int checkAdjacentBlock(World worldIn, BlockPos pos, int currentMinLevel) {
        ...
        if (0 > i) {
            ...
            return 0 <= currentMinLevel && i >= currentMinLevel ? currentMinLevel : i;
        }
    }
