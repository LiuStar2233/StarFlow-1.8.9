...
public class BlockFire extends Block {
    ...
    public IBlockState getActualState(IBlockState state, IBlockAccess worldIn, BlockPos pos) {
        ...
        if (!World.doesBlockHaveSolidTopSurface(worldIn, pos.down()) && !Blocks.fire.canCatchFire(worldIn, pos.down())) {
            boolean flag = 1 == (i + j + k & 1);
            boolean flag1 = 1 == (i / 2 + j / 2 + k / 2 & 1);
            int l = 0;
            ...
            if (!flag && worldIn.isRaining() && this.canDie(worldIn, pos)) {
                ...
                if (!flag) {
                    if (!this.canNeighborCatchFire(worldIn, pos)) {
                        if (!World.doesBlockHaveSolidTopSurface(worldIn, pos.down()) || 3 < i) {
                            worldIn.setBlockToAir(pos);
                        }
                    ...
                    if (!this.canCatchFire(worldIn, pos.down()) && 15 == i && 0 == rand.nextInt(4)) {
                        worldIn.setBlockToAir(pos);
                        return;
                    }
                }
                        ...
                        for (int i1 = -1; 4 >= i1; ++i1) {
                            if (0 != k || 0 != i1 || 0 != l) {
                                ...
                                if (0 < k1) {
                                    ...
                                    if (0 < l1 && rand.nextInt(j1) <= l1 && (!worldIn.isRaining() || !this.canDie(worldIn, blockpos))) {
    ...
    private void catchOnFire(World worldIn, BlockPos pos, int chance, Random random, int age) {
        ...
        if (random.nextInt(chance) < i) {
            ...
            if (5 > random.nextInt(age + 10) && !worldIn.isRainingAt(pos)) {
    ...
    public boolean canCatchFire(IBlockAccess worldIn, BlockPos pos) {
        return 0 < getEncouragement(worldIn.getBlockState(pos).getBlock());
    }
    ...
    public void onBlockAdded(World worldIn, BlockPos pos, IBlockState state) {
        if (0 < worldIn.provider.getDimensionId() || !Blocks.portal.func_176548_d(worldIn, pos)) {
            if (!World.doesBlockHaveSolidTopSurface(worldIn, pos.down()) && !this.canNeighborCatchFire(worldIn, pos)) {
                worldIn.setBlockToAir(pos);
            } else {
