...
public abstract class BlockLeaves extends BlockLeavesBase {
    ...
    public void breakBlock(World worldIn, BlockPos pos, IBlockState state) {
        ...
        if (worldIn.isAreaLoaded(new BlockPos(k - j, l - j, i1 - j), new BlockPos(k + j, l + j, i1 + j))) {
            for (int j1 = -i; i >= j1; ++j1) {
                for (int k1 = -i; i >= k1; ++k1) {
                    for (int l1 = -i; i >= l1; ++l1) {
        ...
        if (!worldIn.isRemote) {
            if (state.getValue(CHECK_DECAY).booleanValue() && state.getValue(DECAYABLE).booleanValue()) {
                ...
                if (worldIn.isAreaLoaded(new BlockPos(k - j, l - j, i1 - j), new BlockPos(k + j, l + j, i1 + j))) {
                    ...
                    for (int i2 = -i; i >= i2; ++i2) {
                        for (int j2 = -i; i >= j2; ++j2) {
                            for (int k2 = -i; i >= k2; ++k2) {
                    ...
                    for (int i3 = 1; 4 >= i3; ++i3) {
                        for (int j3 = -i; i >= j3; ++j3) {
                            for (int k3 = -i; i >= k3; ++k3) {
                                for (int l3 = -i; i >= l3; ++l3) {
                                    if (this.surroundings[(j3 + l1) * k1 + (k3 + l1) * j1 + l3 + l1] == i3 - 1) {
                                        if (-2 == surroundings[(j3 + l1 - 1) * k1 + (k3 + l1) * j1 + l3 + l1]) {
                                        ...
                                        if (-2 == surroundings[(j3 + l1) * k1 + (k3 + l1 - 1) * j1 + l3 + l1]) {
                                        ...
                                        if (-2 == surroundings[(j3 + l1) * k1 + (k3 + l1) * j1 + (l3 + l1 - 1)]) {
    ...
    public void randomDisplayTick(World worldIn, BlockPos pos, IBlockState state, Random rand) {
        if (worldIn.isRainingAt(pos.up()) && !World.doesBlockHaveSolidTopSurface(worldIn, pos.down()) && 1 == rand.nextInt(15)) {
    ...
    public int quantityDropped(Random random) {
        return 0 == random.nextInt(20) ? 1 : 0;
    }
