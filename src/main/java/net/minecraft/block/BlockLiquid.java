...
public abstract class BlockLiquid extends Block {
    ...
    public boolean canCollideCheck(IBlockState state, boolean hitIfLiquid) {
        return hitIfLiquid && 0 == state.getValue(BlockLiquid.LEVEL).intValue();
    }
    ...
    public boolean isBlockSolid(IBlockAccess worldIn, BlockPos pos, EnumFacing side) {
        ...
        return material != this.blockMaterial && (EnumFacing.UP == side || (material != Material.ice && super.isBlockSolid(worldIn, pos, side)));
    }

    public boolean shouldSideBeRendered(IBlockAccess worldIn, BlockPos pos, EnumFacing side) {
        return worldIn.getBlockState(pos).getBlock().getMaterial() != this.blockMaterial && (EnumFacing.UP == side || super.shouldSideBeRendered(worldIn, pos, side));
    }
    ...
    protected Vec3 getFlowVector(IBlockAccess worldIn, BlockPos pos) {
        ...
        if (8 <= worldIn.getBlockState(pos).getValue(BlockLiquid.LEVEL).intValue()) {
    ...
    public void randomDisplayTick(World worldIn, BlockPos pos, IBlockState state, Random rand) {
        ...
        if (0 == rand.nextInt(10) && World.doesBlockHaveSolidTopSurface(worldIn, pos.down())) {
    ...
    public static double getFlowDirection(IBlockAccess worldIn, BlockPos pos, Material materialIn) {
        ...
        return 0.0D == vec3.xCoord && 0.0D == vec3.zCoord ? -1000.0D : MathHelper.atan2(vec3.zCoord, vec3.xCoord) - (Math.PI / 2D);
    }
    ...
    public boolean checkForMixing(World worldIn, BlockPos pos, IBlockState state) {
        if (this.blockMaterial == Material.lava) {
            ...
            for (EnumFacing enumfacing : EnumFacing.values()) {
                if (EnumFacing.DOWN != enumfacing && worldIn.getBlockState(pos.offset(enumfacing)).getBlock().getMaterial() == Material.water) {
                    flag = true;
                    break;
                }
            }
