...
public class BlockOldLeaf extends BlockLeaves {
    public static final PropertyEnum<BlockPlanks.EnumType> VARIANT = PropertyEnum.create("variant", BlockPlanks.EnumType.class, new Predicate<BlockPlanks.EnumType>() {
        public boolean apply(BlockPlanks.EnumType p_apply_1_) {
            return 4 > p_apply_1_.getMetadata();
        }
    });
    ...
    public int getRenderColor(IBlockState state) {
        if (state.getBlock() != this) {
            ...
            return BlockPlanks.EnumType.SPRUCE == blockplanks$enumtype ? ColorizerFoliage.getFoliageColorPine() : (BlockPlanks.EnumType.BIRCH == blockplanks$enumtype ? ColorizerFoliage.getFoliageColorBirch() : super.getRenderColor(state));
        }
    }
    ...
    public int colorMultiplier(IBlockAccess worldIn, BlockPos pos, int renderPass) {
        ...
        if (iblockstate.getBlock() == this) {
            ...
            if (BlockPlanks.EnumType.SPRUCE == blockplanks$enumtype) {
                return ColorizerFoliage.getFoliageColorPine();
            }

            if (BlockPlanks.EnumType.BIRCH == blockplanks$enumtype) {
    ...
    protected void dropApple(World worldIn, BlockPos pos, IBlockState state, int chance) {
        if (BlockPlanks.EnumType.OAK == state.getValue(BlockOldLeaf.VARIANT) && 0 == worldIn.rand.nextInt(chance)) {
            spawnAsEntity(worldIn, pos, new ItemStack(Items.apple, 1, 0));
        }
    }

    protected int getSaplingDropChance(IBlockState state) {
        return BlockPlanks.EnumType.JUNGLE == state.getValue(BlockOldLeaf.VARIANT) ? 40 : super.getSaplingDropChance(state);
    }
    ...
    public IBlockState getStateFromMeta(int meta) {
        return this.getDefaultState().withProperty(VARIANT, this.getWoodType(meta)).withProperty(DECAYABLE, Boolean.valueOf(0 == (meta & 4))).withProperty(CHECK_DECAY, Boolean.valueOf(0 < (meta & 8)));
    }
    ...
    public void harvestBlock(World worldIn, EntityPlayer player, BlockPos pos, IBlockState state, TileEntity te) {
        if (!worldIn.isRemote && null != player.getCurrentEquippedItem() && player.getCurrentEquippedItem().getItem() == Items.shears) {
