...
public class BlockPistonBase extends Block {
    ...
    public void onBlockAdded(World worldIn, BlockPos pos, IBlockState state) {
        if (!worldIn.isRemote && null == worldIn.getTileEntity(pos)) {
            this.checkForMove(worldIn, pos, state);
        }
    }
    ...
    private boolean shouldBeExtended(World worldIn, BlockPos pos, EnumFacing facing) {
        ...
        if (worldIn.isSidePowered(pos, EnumFacing.DOWN)) {
            ...
            for (EnumFacing enumfacing1 : EnumFacing.values()) {
                if (EnumFacing.DOWN != enumfacing1 && worldIn.isSidePowered(blockpos.offset(enumfacing1), enumfacing1)) {
                    return true;
                }
            }
    ...
    public boolean onBlockEventReceived(World worldIn, BlockPos pos, IBlockState state, int eventID, int eventParam) {
        ...
        if (0 == eventID) {
            ...
            if (this.isSticky) {
                ...
                if (!flag1 && block.getMaterial() != Material.air && canPush(block, worldIn, blockpos, enumfacing.getOpposite(), false) && (0 == block.getMobilityFlag() || block == Blocks.piston || block == Blocks.sticky_piston)) {
    ...
    public static EnumFacing getFacing(int meta) {
        int i = meta & 7;
        return 5 < i ? null : EnumFacing.getFront(i);
    }

    public static EnumFacing getFacingFromEntity(World worldIn, BlockPos clickedBlock, EntityLivingBase entityIn) {
        if (2.0F > MathHelper.abs((float) entityIn.posX - (float) clickedBlock.getX()) && 2.0F > MathHelper.abs((float) entityIn.posZ - (float) clickedBlock.getZ())) {
            double d0 = entityIn.posY + (double) entityIn.getEyeHeight();

            if (2.0D < d0 - (double) clickedBlock.getY()) {
                return EnumFacing.UP;
            }

            if (0.0D < (double) clickedBlock.getY() - d0) {
    ...
    public static boolean canPush(Block blockIn, World worldIn, BlockPos pos, EnumFacing direction, boolean allowDestroy) {
        if (blockIn == Blocks.obsidian) {
            ...
            return false;
        } else if (0 <= pos.getY() && (EnumFacing.DOWN != direction || 0 != pos.getY())) {
            if (pos.getY() <= worldIn.getHeight() - 1 && (EnumFacing.UP != direction || pos.getY() != worldIn.getHeight() - 1)) {
                if (blockIn != Blocks.piston && blockIn != Blocks.sticky_piston) {
                    if (-1.0F == blockIn.getBlockHardness(worldIn, pos)) {
                        return false;
                    }

                    if (2 == blockIn.getMobilityFlag()) {
                        return false;
                    }

                    if (1 == blockIn.getMobilityFlag()) {
                        return allowDestroy;
                    }
    ...
    private boolean doMove(World worldIn, BlockPos pos, EnumFacing direction, boolean extending) {
        ...
        if (!blockpistonstructurehelper.canMove()) {
            ...
            for (int l = list1.size() - 1; 0 <= l; --l) {
                worldIn.notifyNeighborsOfStateChange(list1.get(l), ablock[i++]);
            }

            for (int i1 = list.size() - 1; 0 <= i1; --i1) {
    ...
    public IBlockState getStateFromMeta(int meta) {
        return this.getDefaultState().withProperty(FACING, getFacing(meta)).withProperty(EXTENDED, Boolean.valueOf(0 < (meta & 8)));
    }
