...
public class BlockRedstoneComparator extends BlockRedstoneDiode implements ITileEntityProvider {
    ...
    private int calculateOutput(World worldIn, BlockPos pos, IBlockState state) {
        return Mode.SUBTRACT == state.getValue(BlockRedstoneComparator.MODE) ? Math.max(this.calculateInputStrength(worldIn, pos, state) - this.getPowerOnSides(worldIn, pos, state), 0) : this.calculateInputStrength(worldIn, pos, state);
    }
    ...
    protected int calculateInputStrength(World worldIn, BlockPos pos, IBlockState state) {
        ...
        } else if (15 > i && block.isNormalCube()) {
    ...
    private EntityItemFrame findItemFrame(World worldIn, final EnumFacing facing, BlockPos pos) {
        List<EntityItemFrame> list = worldIn.getEntitiesWithinAABB(EntityItemFrame.class, new AxisAlignedBB(pos.getX(), pos.getY(), pos.getZ(), pos.getX() + 1, pos.getY() + 1, pos.getZ() + 1), new Predicate<Entity>() {
            public boolean apply(Entity p_apply_1_) {
                return null != p_apply_1_ && p_apply_1_.getHorizontalFacing() == facing;
            }
        });
    ...
    public boolean onBlockActivated(World worldIn, BlockPos pos, IBlockState state, EntityPlayer playerIn, EnumFacing side, float hitX, float hitY, float hitZ) {
        if (!playerIn.capabilities.allowEdit) {
            ...
            worldIn.playSoundEffect((double) pos.getX() + 0.5D, (double) pos.getY() + 0.5D, (double) pos.getZ() + 0.5D, "random.click", 0.3F, Mode.SUBTRACT == state.getValue(BlockRedstoneComparator.MODE) ? 0.55F : 0.5F);
    ...
    private void onStateChange(World worldIn, BlockPos pos, IBlockState state) {
        ...
        if (j != i || Mode.COMPARE == state.getValue(BlockRedstoneComparator.MODE)) {
    ...
    public boolean onBlockEventReceived(World worldIn, BlockPos pos, IBlockState state, int eventID, int eventParam) {
        ...
        TileEntity tileentity = worldIn.getTileEntity(pos);
        return null != tileentity && tileentity.receiveClientEvent(eventID, eventParam);
    }
    ...
    public IBlockState getStateFromMeta(int meta) {
        return this.getDefaultState().withProperty(FACING, EnumFacing.getHorizontal(meta)).withProperty(POWERED, Boolean.valueOf(0 < (meta & 8))).withProperty(MODE, 0 < (meta & 4) ? BlockRedstoneComparator.Mode.SUBTRACT : BlockRedstoneComparator.Mode.COMPARE);
    }
    ...
    public int getMetaFromState(IBlockState state) {
        ...
        if (Mode.SUBTRACT == state.getValue(BlockRedstoneComparator.MODE)) {
            i |= 4;
        }
