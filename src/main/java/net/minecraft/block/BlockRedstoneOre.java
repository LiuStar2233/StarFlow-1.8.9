...
public class BlockRedstoneOre extends Block {
    ...
    private void spawnParticles(World worldIn, BlockPos pos) {
        ...
        for (int i = 0; 6 > i; ++i) {
            ...
            if (0 == i && !worldIn.getBlockState(pos.up()).getBlock().isOpaqueCube()) {
                d2 = (double) pos.getY() + d0 + 1.0D;
            }

            if (1 == i && !worldIn.getBlockState(pos.down()).getBlock().isOpaqueCube()) {
                d2 = (double) pos.getY() - d0;
            }

            if (2 == i && !worldIn.getBlockState(pos.south()).getBlock().isOpaqueCube()) {
                d3 = (double) pos.getZ() + d0 + 1.0D;
            }

            if (3 == i && !worldIn.getBlockState(pos.north()).getBlock().isOpaqueCube()) {
                d3 = (double) pos.getZ() - d0;
            }

            if (4 == i && !worldIn.getBlockState(pos.east()).getBlock().isOpaqueCube()) {
                d1 = (double) pos.getX() + d0 + 1.0D;
            }

            if (5 == i && !worldIn.getBlockState(pos.west()).getBlock().isOpaqueCube()) {
                d1 = (double) pos.getX() - d0;
            }

            if (d1 < (double) pos.getX() || d1 > (double) (pos.getX() + 1) || 0.0D > d2 || d2 > (double) (pos.getY() + 1) || d3 < (double) pos.getZ() || d3 > (double) (pos.getZ() + 1)) {
