...
public class BlockSkull extends BlockContainer {
    ...
    private static final Predicate<BlockWorldState> IS_WITHER_SKELETON = new Predicate<BlockWorldState>() {
        public boolean apply(BlockWorldState p_apply_1_) {
            return null != p_apply_1_.getBlockState() && p_apply_1_.getBlockState().getBlock() == Blocks.skull && p_apply_1_.getTileEntity() instanceof TileEntitySkull && 1 == ((TileEntitySkull) p_apply_1_.getTileEntity()).getSkullType();
        }
    };
        ...
        if (!worldIn.isRemote) {
            if (!state.getValue(NODROP).booleanValue()) {
                ...
                if (tileentity instanceof TileEntitySkull) {
                    ...
                    if (3 == tileentityskull.getSkullType() && null != tileentityskull.getPlayerProfile()) {
    ...
    public boolean canDispenserPlace(World worldIn, BlockPos pos, ItemStack stack) {
        return 1 == stack.getMetadata() && 2 <= pos.getY() && EnumDifficulty.PEACEFUL != worldIn.getDifficulty() && !worldIn.isRemote && null != getWitherBasePattern().match(worldIn, pos);
    }

    public void checkWitherSpawn(World worldIn, BlockPos pos, TileEntitySkull te) {
        if (1 == te.getSkullType() && 2 <= pos.getY() && EnumDifficulty.PEACEFUL != worldIn.getDifficulty() && !worldIn.isRemote) {
            ...
            if (null != blockpattern$patternhelper) {
                ...
                entitywither.setLocationAndAngles((double) blockpos1.getX() + 0.5D, (double) blockpos1.getY() + 0.55D, (double) blockpos1.getZ() + 0.5D, EnumFacing.Axis.X == blockpattern$patternhelper.getFinger().getAxis() ? 0.0F : 90.0F, 0.0F);
                entitywither.renderYawOffset = EnumFacing.Axis.X == blockpattern$patternhelper.getFinger().getAxis() ? 0.0F : 90.0F;
    ...
    public IBlockState getStateFromMeta(int meta) {
        return this.getDefaultState().withProperty(FACING, EnumFacing.getFront(meta & 7)).withProperty(NODROP, Boolean.valueOf(0 < (meta & 8)));
    }
