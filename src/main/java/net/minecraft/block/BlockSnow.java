...
public class BlockSnow extends Block {
    ...
    public boolean isPassable(IBlockAccess worldIn, BlockPos pos) {
        return 5 > worldIn.getBlockState(pos).getValue(BlockSnow.LAYERS).intValue();
    }
    ...
    public boolean canPlaceBlockAt(World worldIn, BlockPos pos) {
        ...
        return block != Blocks.ice && block != Blocks.packed_ice && (block.getMaterial() == Material.leaves || (block == this && 7 <= iblockstate.getValue(BlockSnow.LAYERS).intValue() || block.isOpaqueCube() && block.blockMaterial.blocksMovement()));
    }
    ...
    public void updateTick(World worldIn, BlockPos pos, IBlockState state, Random rand) {
        if (11 < worldIn.getLightFor(EnumSkyBlock.BLOCK, pos)) {
    ...
    public boolean shouldSideBeRendered(IBlockAccess worldIn, BlockPos pos, EnumFacing side) {
        return EnumFacing.UP == side || super.shouldSideBeRendered(worldIn, pos, side);
    }
    ...
    public boolean isReplaceable(World worldIn, BlockPos pos) {
        return 1 == worldIn.getBlockState(pos).getValue(BlockSnow.LAYERS).intValue();
    }
