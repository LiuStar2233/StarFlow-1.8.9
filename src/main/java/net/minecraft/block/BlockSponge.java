...
public class BlockSponge extends Block {
    ...
    public IBlockState getStateFromMeta(int meta) {
        return this.getDefaultState().withProperty(WET, Boolean.valueOf(1 == (meta & 1)));
    }
    ...
    public void randomDisplayTick(World worldIn, BlockPos pos, IBlockState state, Random rand) {
        if (state.getValue(WET).booleanValue()) {
            ...
            if (EnumFacing.UP != enumfacing && !World.doesBlockHaveSolidTopSurface(worldIn, pos.offset(enumfacing))) {
                double d0 = pos.getX();
                double d1 = pos.getY();
                double d2 = pos.getZ();
                ...
                if (EnumFacing.DOWN == enumfacing) {
                    ...
                    if (EnumFacing.Axis.X == enumfacing.getAxis()) {
                        d2 += rand.nextDouble();
