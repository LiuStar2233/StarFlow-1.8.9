...
public abstract class BlockStoneSlab extends BlockSlab {
    ...
    public String getUnlocalizedName(int meta) {
        return this.getUnlocalizedName() + "." + BlockStoneSlab.EnumType.byMetadata(meta).getUnlocalizedName();
    }
    ...
    public void getSubBlocks(Item itemIn, CreativeTabs tab, List<ItemStack> list) {
        if (itemIn != Item.getItemFromBlock(Blocks.double_stone_slab)) {
            for (BlockStoneSlab.EnumType blockstoneslab$enumtype : BlockStoneSlab.EnumType.values()) {
                if (EnumType.WOOD != blockstoneslab$enumtype) {
    ...
    public IBlockState getStateFromMeta(int meta) {
        ...
        if (this.isDouble()) {
            iblockstate = iblockstate.withProperty(SEAMLESS, Boolean.valueOf(0 != (meta & 8)));
        } else {
            iblockstate = iblockstate.withProperty(HALF, 0 == (meta & 8) ? BlockSlab.EnumBlockHalf.BOTTOM : BlockSlab.EnumBlockHalf.TOP);
        }
    ...
    public int getMetaFromState(IBlockState state) {
        ...
        } else if (EnumBlockHalf.TOP == state.getValue(BlockSlab.HALF)) {
            i |= 8;
        }
