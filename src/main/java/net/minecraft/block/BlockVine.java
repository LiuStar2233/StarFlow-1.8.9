...
public class BlockVine extends Block {
    ...
    private boolean recheckGrownSides(World worldIn, BlockPos pos, IBlockState state) {
        ...
        if (0 == BlockVine.getNumGrownFaces(state)) {
            return false;
        } else {
    ...
    public void updateTick(World worldIn, BlockPos pos, IBlockState state, Random rand) {
        if (!worldIn.isRemote) {
            if (0 == worldIn.rand.nextInt(4)) {
                ...
                if (EnumFacing.UP == enumfacing1 && 255 > pos.getY() && worldIn.isAirBlock(blockpos1)) {
                    if (!flag) {
    ...
    public IBlockState getStateFromMeta(int meta) {
        return this.getDefaultState().withProperty(SOUTH, Boolean.valueOf(0 < (meta & 1))).withProperty(WEST, Boolean.valueOf(0 < (meta & 2))).withProperty(NORTH, Boolean.valueOf(0 < (meta & 4))).withProperty(EAST, Boolean.valueOf(0 < (meta & 8)));
    }
