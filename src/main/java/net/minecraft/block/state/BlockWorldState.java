...
public class BlockWorldState {
    ...
    public IBlockState getBlockState() {
        if (null == state && (this.field_181628_c || this.world.isBlockLoaded(this.pos))) {
    ...
    public TileEntity getTileEntity() {
        if (null == tileEntity && !this.tileEntityInitialized) {
    ...
    public static Predicate<BlockWorldState> hasState(final Predicate<IBlockState> predicatesIn) {
        return new Predicate<BlockWorldState>() {
            public boolean apply(BlockWorldState p_apply_1_) {
                return null != p_apply_1_ && predicatesIn.apply(p_apply_1_.getBlockState());
            }
        };
    }
}