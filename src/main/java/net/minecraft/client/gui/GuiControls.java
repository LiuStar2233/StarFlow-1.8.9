...
public class GuiControls extends GuiScreen {
    ...
    protected void actionPerformed(GuiButton button) throws IOException {
        ...
        } else if (201 == button.id) {
            for (KeyBinding keybinding : this.mc.gameSettings.keyBindings) {
                keybinding.setKeyCode(keybinding.getKeyCodeDefault());
            }

            KeyBinding.resetKeyBindingArrayAndHash();
        } else if (100 > button.id && button instanceof GuiOptionButton) {
    ...
    protected void mouseClicked(int mouseX, int mouseY, int mouseButton) throws IOException {
        if (null != buttonId) {
            ...
            this.buttonId = null;
            KeyBinding.resetKeyBindingArrayAndHash();
        } else if (0 != mouseButton || !this.keyBindingList.mouseClicked(mouseX, mouseY, mouseButton)) {
    ...
    protected void mouseReleased(int mouseX, int mouseY, int state) {
        if (0 != state || !this.keyBindingList.mouseReleased(mouseX, mouseY, state)) {
