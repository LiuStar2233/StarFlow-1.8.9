...
public class GuiEnchantment extends GuiContainer {
    ...
    protected void mouseClicked(int mouseX, int mouseY, int mouseButton) throws IOException {
        ...
        for (int k = 0; 3 > k; ++k) {
            ...
            if (0 <= l && 0 <= i1 && 108 > l && 19 > i1 && this.container.enchantItem(this.mc.thePlayer, k)) {
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        ...
        for (int j = 0; 3 > j; ++j) {
            ...
            if (this.isPointInRegion(60, 14 + 19 * j, 108, 17, mouseX, mouseY) && 0 < k && 0 <= l) {
                List<String> list = Lists.newArrayList();

                if (0 <= l && null != Enchantment.getEnchantmentById(l & 255)) {
    ...
    public void func_147068_g() {
        ...
        for (int i = 0; 3 > i; ++i) {
            if (0 != container.enchantLevels[i]) {
                flag = true;
                break;
            }
        }
