...
public class GuiScreenBook extends GuiScreen {
    ...
    private void updateButtons() {
        ...
        this.buttonPreviousPage.visible = !this.bookGettingSigned && 0 < currPage;
        ...
        if (this.bookIsUnsigned) {
            ...
            this.buttonFinalize.enabled = 0 < bookTitle.trim().length();
        }
    }
    ...
    protected void actionPerformed(GuiButton button) throws IOException {
        if (button.enabled) {
            ...
            } else if (3 == button.id && this.bookIsUnsigned) {
            ...
            } else if (5 == button.id && this.bookGettingSigned) {
                this.sendBookToServer(true);
                this.mc.displayGuiScreen(null);
            } else if (4 == button.id && this.bookGettingSigned) {
    ...
    private void addNewPage() {
        if (null != bookPages && 50 > bookPages.tagCount()) {
            this.bookPages.appendTag(new NBTTagString(""));
            ++this.bookTotalPages;
    ...
    private void keyTypedInTitle(char p_146460_1_, int p_146460_2_) throws IOException {
        switch (p_146460_2_) {
            ...
            default:
                if (16 > bookTitle.length() && ChatAllowedCharacters.isAllowedCharacter(p_146460_1_)) {
    ...
    private String pageGetCurrent() {
        return null != bookPages && 0 <= currPage && this.currPage < this.bookPages.tagCount() ? this.bookPages.getStringTagAt(this.currPage) : "";
    }
    ...
    private void pageSetCurrent(String p_146457_1_) {
        if (null != bookPages && 0 <= currPage && this.currPage < this.bookPages.tagCount()) {
            ...
            if (this.bookIsUnsigned) {
                ...
                if (ItemEditableBook.validBookTagContents(this.bookObj.getTagCompound())) {
                    try {
                        ...
                        this.field_175386_A = null != ichatcomponent ? GuiUtilRenderComponents.splitText(ichatcomponent, 116, this.fontRendererObj, true, true) : null;
    ...
    protected boolean handleComponentClick(IChatComponent component) {
        ClickEvent clickevent = null == component ? null : component.getChatStyle().getChatClickEvent();

        if (null == clickevent) {
            return false;
        } else if (ClickEvent.Action.CHANGE_PAGE == clickevent.getAction()) {
            String s = clickevent.getValue();

            try {
                int i = Integer.parseInt(s) - 1;

                if (0 <= i && i < this.bookTotalPages && i != this.currPage) {
                    this.currPage = i;
                    this.updateButtons();
                    return true;
                }
            ...
            if (flag && ClickEvent.Action.RUN_COMMAND == clickevent.getAction()) {
                this.mc.displayGuiScreen(null);
            }
    ...
    public IChatComponent func_175385_b(int p_175385_1_, int p_175385_2_) {
        if (null == field_175386_A) {
            ...
            if (0 <= i && 0 <= j) {
                ...
                if (116 >= i && j < this.mc.fontRendererObj.FONT_HEIGHT * k + k) {
                    int l = j / this.mc.fontRendererObj.FONT_HEIGHT;

                    if (0 <= l && l < this.field_175386_A.size()) {
