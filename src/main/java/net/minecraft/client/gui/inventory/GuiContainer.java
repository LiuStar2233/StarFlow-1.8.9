...
public abstract class GuiContainer extends GuiScreen {
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        ...
        ItemStack itemstack = null == draggedStack ? inventoryplayer.getItemStack() : this.draggedStack;

        if (null != itemstack) {
            final int j2 = 8;
            int k2 = null == draggedStack ? 8 : 16;
            String s = null;

            if (null != draggedStack && this.isRightMouseClick) {
                itemstack = itemstack.copy();
                itemstack.stackSize = MathHelper.ceiling_float_int((float) itemstack.stackSize / 2.0F);
            } else if (this.dragSplitting && 1 < dragSplittingSlots.size()) {
                itemstack = itemstack.copy();
        ...
        if (null == inventoryplayer.getItemStack() && null != theSlot && this.theSlot.getHasStack()) {
    ...
    private void drawItemStack(ItemStack stack, int x, int y, String altText) {
        ...
        this.itemRender.renderItemOverlayIntoGUI(this.fontRendererObj, stack, x, y - (null == draggedStack ? 0 : 8), altText);
        this.zLevel = 0.0F;
    ...
    private void drawSlot(Slot slotIn) {
        ...
        boolean flag = false;
        boolean flag1 = slotIn == this.clickedSlot && null != draggedStack && !this.isRightMouseClick;
        ItemStack itemstack1 = this.mc.thePlayer.inventory.getItemStack();
        String s = null;

        if (slotIn == this.clickedSlot && null != draggedStack && this.isRightMouseClick && null != itemstack) {
            itemstack = itemstack.copy();
            itemstack.stackSize /= 2;
        } else if (this.dragSplitting && this.dragSplittingSlots.contains(slotIn) && null != itemstack1) {
            if (1 == dragSplittingSlots.size()) {
                return;
            }
            ...
            if (Container.canAddItemToSlot(slotIn, itemstack1, true) && this.inventorySlots.canDragIntoSlot(slotIn)) {
                ...
                flag = true;
                Container.computeStackSize(this.dragSplittingSlots, this.dragSplittingLimit, itemstack, null == slotIn.getStack() ? 0 : slotIn.getStack().stackSize);
    ...
    private void updateDragSplitting() {
        ...
        if (null != itemstack && this.dragSplitting) {
            this.dragSplittingRemnant = itemstack.stackSize;

            for (Slot slot : this.dragSplittingSlots) {
                ItemStack itemstack1 = itemstack.copy();
                int i = null == slot.getStack() ? 0 : slot.getStack().stackSize;
    ...
    protected void mouseClicked(int mouseX, int mouseY, int mouseButton) throws IOException {
        ...
        this.doubleClick = this.lastClickSlot == slot && 250L > i - lastClickTime && this.lastClickButton == mouseButton;
        this.ignoreMouseUp = false;

        if (0 == mouseButton || 1 == mouseButton || flag) {
            int j = this.guiLeft;
            int k = this.guiTop;
            ...
            if (this.mc.gameSettings.touchscreen && flag1 && null == mc.thePlayer.inventory.getItemStack()) {
            ...
            if (-1 != l) {
                if (this.mc.gameSettings.touchscreen) {
                    if (null != slot && slot.getHasStack()) {
                        ...
                        this.isRightMouseClick = 1 == mouseButton;
                    } else {
                        this.clickedSlot = null;
                    }
                } else if (!this.dragSplitting) {
                    if (null == mc.thePlayer.inventory.getItemStack()) {
                        if (mouseButton == this.mc.gameSettings.keyBindPickBlock.getKeyCode() + 100) {
                            this.handleMouseClick(slot, l, mouseButton, 3);
                        } else {
                            boolean flag2 = -999 != l && (Keyboard.isKeyDown(42) || Keyboard.isKeyDown(54));
                            int i1 = 0;

                            if (flag2) {
                                this.shiftClickedSlot = null != slot && slot.getHasStack() ? slot.getStack() : null;
                                i1 = 1;
                            } else if (-999 == l) {
                                i1 = 4;
                            }
    ...
    protected void mouseClickMove(int mouseX, int mouseY, int clickedMouseButton, long timeSinceLastClick) {
        ...
        if (null != clickedSlot && this.mc.gameSettings.touchscreen) {
            if (0 == clickedMouseButton || 1 == clickedMouseButton) {
                if (null == draggedStack) {
                    if (slot != this.clickedSlot && null != clickedSlot.getStack()) {
                        this.draggedStack = this.clickedSlot.getStack().copy();
                    }
                } else if (1 < draggedStack.stackSize && null != slot && Container.canAddItemToSlot(slot, this.draggedStack, false)) {
                    long i = Minecraft.getSystemTime();

                    if (this.currentDragTargetSlot == slot) {
                        if (500L < i - dragItemDropDelay) {
        ...
        } else if (this.dragSplitting && null != slot && null != itemstack && itemstack.stackSize > this.dragSplittingSlots.size() && Container.canAddItemToSlot(slot, itemstack, true) && slot.isItemValid(itemstack) && this.inventorySlots.canDragIntoSlot(slot)) {
    ...
    protected void mouseReleased(int mouseX, int mouseY, int state) {
        ...
        if (this.doubleClick && null != slot && 0 == state && this.inventorySlots.canMergeSlot(null, slot)) {
            if (isShiftKeyDown()) {
                if (null != slot && null != slot.inventory && null != shiftClickedSlot) {
                    for (Slot slot2 : this.inventorySlots.inventorySlots) {
                        if (null != slot2 && slot2.canTakeStack(this.mc.thePlayer) && slot2.getHasStack() && slot2.inventory == slot.inventory && Container.canAddItemToSlot(slot2, this.shiftClickedSlot, true)) {
            ...
            } else if (null != mc.thePlayer.inventory.getItemStack()) {
                if (state == this.mc.gameSettings.keyBindPickBlock.getKeyCode() + 100) {
                    this.handleMouseClick(slot, k, state, 3);
                } else {
                    boolean flag1 = -999 != k && (Keyboard.isKeyDown(42) || Keyboard.isKeyDown(54));
    ...
    protected void keyTyped(char typedChar, int keyCode) throws IOException {
        if (1 == keyCode || keyCode == this.mc.gameSettings.keyBindInventory.getKeyCode()) {
            this.mc.thePlayer.closeScreen();
        }

        this.checkHotbarKeys(keyCode);

        if (null != theSlot && this.theSlot.getHasStack()) {
    ...
    protected boolean checkHotbarKeys(int keyCode) {
        if (null == mc.thePlayer.inventory.getItemStack() && null != theSlot) {
            for (int i = 0; 9 > i; ++i) {
