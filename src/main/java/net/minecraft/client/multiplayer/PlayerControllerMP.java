...
public class PlayerControllerMP {
    ...
    public boolean isSpectator() {
        return WorldSettings.GameType.SPECTATOR == currentGameType;
    }
    ...
    public boolean onPlayerDestroyBlock(BlockPos pos, EnumFacing side) {
        if (this.currentGameType.isAdventure()) {
            if (WorldSettings.GameType.SPECTATOR == currentGameType) {
                return false;
            }
        ...
        if (this.currentGameType.isCreative() && null != mc.thePlayer.getHeldItem() && this.mc.thePlayer.getHeldItem().getItem() instanceof ItemSword) {
            return false;
        } else {
    ...
    public boolean clickBlock(BlockPos loc, EnumFacing face) {
        ...
        if (!this.mc.theWorld.getWorldBorder().contains(loc)) {
            ...
            if (this.currentGameType.isCreative()) {
                ...
                if (flag && 0.0F == curBlockDamageMP) {
                    block1.onBlockClicked(this.mc.theWorld, loc, this.mc.thePlayer);
                }

                if (flag && 1.0F <= block1.getPlayerRelativeBlockHardness(mc.thePlayer, mc.thePlayer.worldObj, loc)) {
    ...
    public boolean onPlayerDamageBlock(BlockPos posBlock, EnumFacing directionFacing) {
        ...
        if (0 < blockHitDelay) {
            ...
            if (block.getMaterial() == Material.air) {
                ...
                if (0.0F == stepSoundTickCounter % 4.0F) {
    ...
    private boolean isHittingPosition(BlockPos pos) {
        ...
        boolean flag = null == currentItemHittingBlock && null == itemstack;
    ...
    public boolean onPlayerRightClick(EntityPlayerSP player, WorldClient worldIn, ItemStack heldStack, BlockPos hitPos, EnumFacing side, Vec3 hitVec) {
        ...
        if (!this.mc.theWorld.getWorldBorder().contains(hitPos)) {
            ...
            if (WorldSettings.GameType.SPECTATOR != currentGameType) {
                ...
                if ((!player.isSneaking() || null == player.getHeldItem()) && iblockstate.getBlock().onBlockActivated(worldIn, hitPos, iblockstate, player, side, f, f1, f2)) {
                    flag = true;
                }

                if (!flag && null != heldStack && heldStack.getItem() instanceof ItemBlock) {
            ...
            if (!flag && WorldSettings.GameType.SPECTATOR != currentGameType) {
                if (null == heldStack) {
                    return false;
    ...
    public boolean sendUseItem(EntityPlayer playerIn, World worldIn, ItemStack itemStackIn) {
        if (WorldSettings.GameType.SPECTATOR == currentGameType) {
            ...
            if (itemstack != itemStackIn || null != itemstack && itemstack.stackSize != i) {
    ...
    public boolean interactWithEntitySendPacket(EntityPlayer playerIn, Entity targetEntity) {
        ...
        return WorldSettings.GameType.SPECTATOR != currentGameType && playerIn.interactWith(targetEntity);
    }
    ...
    public boolean isPlayerRightClickingOnEntity(EntityPlayer player, Entity entityIn, MovingObjectPosition movingObject) {
        ...
        return WorldSettings.GameType.SPECTATOR != currentGameType && entityIn.interactAt(player, vec3);
    }
    ...
    public void sendPacketDropItem(ItemStack itemStackIn) {
        if (this.currentGameType.isCreative() && null != itemStackIn) {
