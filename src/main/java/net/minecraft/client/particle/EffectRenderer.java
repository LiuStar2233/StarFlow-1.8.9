...
public class EffectRenderer {
    ...
    public void addEffect(EntityFX effect) {
        ...
        int j = 1.0F != effect.getAlpha() ? 0 : 1;

        if (4000 <= fxLayers[i][j].size()) {
    ...
    private void tickParticle(final EntityFX particle) {
        try {
            ...
            crashreportcategory.addCrashSectionCallable("Particle Type", new Callable<String>() {
                public String call() throws Exception {
                    return 0 == i ? "MISC_TEXTURE" : (1 == i ? "TERRAIN_TEXTURE" : (3 == i ? "ENTITY_PARTICLE_TEXTURE" : "Unknown - " + i));
                }
            });
                ...
                if (!this.fxLayers[i][j].isEmpty()) {
                    ...
                    for (int k = 0; k < this.fxLayers[i][j].size(); ++k) {
                        ...
                        try {
                            ...
                            crashreportcategory.addCrashSectionCallable("Particle Type", new Callable<String>() {
                                public String call() throws Exception {
                                    return 0 == i_f ? "MISC_TEXTURE" : (1 == i_f ? "TERRAIN_TEXTURE" : (3 == i_f ? "ENTITY_PARTICLE_TEXTURE" : "Unknown - " + i_f));
                                }
                            });
                            throw new ReportedException(crashreport);
                        }
    ...
    public void addBlockHitEffects(BlockPos pos, EnumFacing side) {
        ...
        if (-1 != block.getRenderType()) {
            ...
            if (EnumFacing.WEST == side) {
                d0 = (double) i + block.getBlockBoundsMinX() - (double) f;
            }

            if (EnumFacing.EAST == side) {
