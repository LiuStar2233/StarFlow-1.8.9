...
public class BlockModelRenderer {
    ...
    public boolean renderModel(IBlockAccess blockAccessIn, IBakedModel modelIn, IBlockState blockStateIn, BlockPos blockPosIn, WorldRenderer worldRendererIn, boolean checkSides) {
        boolean flag = Minecraft.isAmbientOcclusionEnabled() && 0 == blockStateIn.getBlock().getLightValue() && modelIn.isAmbientOcclusion();
    ...
    private void renderModelAmbientOcclusionQuads(IBlockAccess blockAccessIn, Block blockIn, BlockPos blockPosIn, WorldRenderer worldRendererIn, List<BakedQuad> listQuadsIn, float[] quadBounds, BitSet boundsFlags, BlockModelRenderer.AmbientOcclusionFace aoFaceIn) {
        ...
        if (Block.EnumOffsetType.NONE != block$enumoffsettype) {
            ...
            if (Block.EnumOffsetType.XYZ == block$enumoffsettype) {
    ...
    private void fillQuadBounds(Block blockIn, int[] vertexData, EnumFacing facingIn, float[] quadBounds, BitSet boundsFlags) {
        ...
        switch (facingIn) {
            case DOWN:
                boundsFlags.set(1, 1.0E-4F <= f || 1.0E-4F <= f2 || 0.9999F >= f3 || 0.9999F >= f5);
                boundsFlags.set(0, (1.0E-4F > f1 || blockIn.isFullCube()) && f1 == f4);
                break;

            case UP:
                boundsFlags.set(1, 1.0E-4F <= f || 1.0E-4F <= f2 || 0.9999F >= f3 || 0.9999F >= f5);
                boundsFlags.set(0, (0.9999F < f4 || blockIn.isFullCube()) && f1 == f4);
                break;

            case NORTH:
                boundsFlags.set(1, 1.0E-4F <= f || 1.0E-4F <= f1 || 0.9999F >= f3 || 0.9999F >= f4);
                boundsFlags.set(0, (1.0E-4F > f2 || blockIn.isFullCube()) && f2 == f5);
                break;

            case SOUTH:
                boundsFlags.set(1, 1.0E-4F <= f || 1.0E-4F <= f1 || 0.9999F >= f3 || 0.9999F >= f4);
                boundsFlags.set(0, (0.9999F < f5 || blockIn.isFullCube()) && f2 == f5);
                break;

            case WEST:
                boundsFlags.set(1, 1.0E-4F <= f1 || 1.0E-4F <= f2 || 0.9999F >= f4 || 0.9999F >= f5);
                boundsFlags.set(0, (1.0E-4F > f || blockIn.isFullCube()) && f == f3);
                break;

            case EAST:
                boundsFlags.set(1, 1.0E-4F <= f1 || 1.0E-4F <= f2 || 0.9999F >= f4 || 0.9999F >= f5);
                boundsFlags.set(0, (0.9999F < f3 || blockIn.isFullCube()) && f == f3);
        }
    }
