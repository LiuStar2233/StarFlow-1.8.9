...
public class RenderGlobal implements IWorldAccess, IResourceManagerReloadListener {
    ...
    public void makeEntityOutlineShader() {
        if (OpenGlHelper.shadersSupported) {
            if (null == ShaderLinkHelper.getStaticShaderLinkHelper()) {
    ...
    protected boolean isRenderEntityOutlines() {
        return null != entityOutlineFramebuffer && null != entityOutlineShader && null != mc.thePlayer && this.mc.thePlayer.isSpectator() && this.mc.gameSettings.keyBindSpectatorOutlines.isKeyDown();
    }
    ...
    private void renderSky(WorldRenderer worldRendererIn, float posY, boolean reverseX) {
        ...
        for (int k = -384; 384 >= k; k += 64) {
            for (int l = -384; 384 >= l; l += 64) {
                float f = (float) k;
                float f1 = (float) (k + 64);
    ...
    public void renderEntities(Entity renderViewEntity, ICamera camera, float partialTicks) {
        if (0 < renderEntitiesStartupCounter) {
            ...
            if (this.isRenderEntityOutlines()) {
                ...
                for (int j = 0; j < list.size(); ++j) {
                    ...
                    if ((entity3 != this.mc.getRenderViewEntity() || 0 != mc.gameSettings.thirdPersonView || flag) && flag1) {
                ...
                if (!classinheritancemultimap.isEmpty()) {
                    ...
                    while (true) {
                        ...
                        while (true) {
                            ...
                            if ((entity2 != this.mc.getRenderViewEntity() || 0 != mc.gameSettings.thirdPersonView || flag3) && (0.0D > entity2.posY || 256.0D <= entity2.posY || this.theWorld.isBlockLoaded(new BlockPos(entity2)))) {
                                ++this.countEntitiesRendered;
            ...
            for (DestroyBlockProgress destroyblockprogress : this.damagedBlocks.values()) {
                ...
                if (tileentity1 instanceof TileEntityChest) {
                    ...
                    if (null != tileentitychest.adjacentChestXNeg) {
                        blockpos = blockpos.offset(EnumFacing.WEST);
                        tileentity1 = this.theWorld.getTileEntity(blockpos);
                    } else if (null != tileentitychest.adjacentChestZNeg) {
                ...
                if (null != tileentity1 && (block instanceof BlockChest || block instanceof BlockEnderChest || block instanceof BlockSign || block instanceof BlockSkull)) {
    ...
    public void setupTerrain(Entity viewEntity, double partialTicks, ICamera camera, int frameCount, boolean playerSpectator) {
        ...
        if (this.frustumUpdatePosChunkX != viewEntity.chunkCoordX || this.frustumUpdatePosChunkY != viewEntity.chunkCoordY || this.frustumUpdatePosChunkZ != viewEntity.chunkCoordZ || 16.0D < d0 * d0 + d1 * d1 + d2 * d2) {
        ...
        if (null != debugFixedClippingHelper) {
        ...
        boolean flag = null != debugFixedClippingHelper;
        ...
        if (!flag && this.displayListEntitiesDirty) {
            ...
            if (null != renderchunk) {
                ...
                for (int j = -this.renderDistanceChunks; j <= this.renderDistanceChunks; ++j) {
                    for (int k = -this.renderDistanceChunks; k <= this.renderDistanceChunks; ++k) {
                        ...
                        if (null != renderchunk1 && camera.isBoundingBoxInFrustum(renderchunk1.boundingBox)) {
            ...
            while (!queue.isEmpty()) {
                ...
                for (EnumFacing enumfacing1 : EnumFacing.values()) {
                    ...
                    if ((!flag1 || !renderglobal$containerlocalrenderinformation1.setFacing.contains(enumfacing1.getOpposite())) && (!flag1 || null == enumfacing2 || renderchunk3.getCompiledChunk().isVisible(enumfacing2.getOpposite(), enumfacing1)) && null != renderchunk2 && renderchunk2.setFrameIndex(frameCount) && camera.isBoundingBoxInFrustum(renderchunk2.boundingBox)) {
    ...
    private boolean isPositionInRenderChunk(BlockPos pos, RenderChunk renderChunkIn) {
        ...
        return 16 >= MathHelper.abs_int(pos.getX() - blockpos.getX()) && (16 >= MathHelper.abs_int(pos.getY() - blockpos.getY()) && 16 >= MathHelper.abs_int(pos.getZ() - blockpos.getZ()));
    }
    ...
    private RenderChunk getRenderChunkOffset(BlockPos playerPos, RenderChunk renderChunkBase, EnumFacing facing) {
        ...
        return MathHelper.abs_int(playerPos.getX() - blockpos.getX()) > this.renderDistanceChunks * 16 ? null : (0 <= blockpos.getY() && 256 > blockpos.getY() ? (MathHelper.abs_int(playerPos.getZ() - blockpos.getZ()) > this.renderDistanceChunks * 16 ? null : this.viewFrustum.getRenderChunk(blockpos)) : null);
    }
    ...
    protected Vector3f getViewVector(Entity entityIn, double partialTicks) {
        ...
        if (2 == Minecraft.getMinecraft().gameSettings.thirdPersonView) {
            f += 180.0F;
        }
    ...
    public int renderBlockLayer(EnumWorldBlockLayer blockLayerIn, double partialTicks, int pass, Entity entityIn) {
        ...
        if (EnumWorldBlockLayer.TRANSLUCENT == blockLayerIn) {
            this.mc.mcProfiler.startSection("translucent_sort");
            double d0 = entityIn.posX - this.prevRenderSortX;
            double d1 = entityIn.posY - this.prevRenderSortY;
            double d2 = entityIn.posZ - this.prevRenderSortZ;

            if (1.0D < d0 * d0 + d1 * d1 + d2 * d2) {
                ...
                for (RenderGlobal.ContainerLocalRenderInformation renderglobal$containerlocalrenderinformation : this.renderInfos) {
                    if (renderglobal$containerlocalrenderinformation.renderChunk.compiledChunk.isLayerStarted(blockLayerIn) && 15 > k++) {
        ...
        int l = 0;
        boolean flag = EnumWorldBlockLayer.TRANSLUCENT == blockLayerIn;
    ...
    public void renderSky(float partialTicks, int pass) {
        if (1 == mc.theWorld.provider.getDimensionId()) {
            this.renderSkyEnd();
            ...
            if (null != afloat) {
                ...
                GlStateManager.pushMatrix();
                GlStateManager.rotate(90.0F, 1.0F, 0.0F, 0.0F);
                GlStateManager.rotate(0.0F > MathHelper.sin(theWorld.getCelestialAngleRadians(partialTicks)) ? 180.0F : 0.0F, 0.0F, 0.0F, 1.0F);
                GlStateManager.rotate(90.0F, 0.0F, 0.0F, 1.0F);
                float f6 = afloat[0];
                float f7 = afloat[1];
                float f8 = afloat[2];
    ...
    public void renderClouds(float partialTicks, int pass) {
        if (this.mc.theWorld.provider.isSurfaceWorld()) {
            if (2 == mc.gameSettings.shouldRenderClouds()) {
                ...
                for (int i1 = -256; 256 > i1; i1 += 32) {
                    for (int j1 = -256; 256 > j1; j1 += 32) {
    ...
    private void renderCloudsFancy(float partialTicks, int pass) {
        ...
        for (int i1 = 0; 2 > i1; ++i1) {
            ...
            for (int j1 = -3; 4 >= j1; ++j1) {
                for (int k1 = -3; 4 >= k1; ++k1) {
    ...
    public void drawSelectionBox(EntityPlayer player, MovingObjectPosition movingObjectPositionIn, int execute, float partialTicks) {
        if (0 == execute && MovingObjectPosition.MovingObjectType.BLOCK == movingObjectPositionIn.typeOfHit) {
            GlStateManager.enableBlend();
            GlStateManager.tryBlendFuncSeparate(770, 771, 1, 0);
    ...
    private EntityFX spawnEntityFX(int particleID, boolean ignoreRange, double xCoord, double yCoord, double zCoord, double xOffset, double yOffset, double zOffset, int... parameters) {
        if (null != mc && null != mc.getRenderViewEntity() && null != mc.effectRenderer) {
            int i = this.mc.gameSettings.particleSetting;

            if (1 == i && 0 == theWorld.rand.nextInt(3)) {
                i = 2;
            }
            ...
            if (ignoreRange) {
                ...
                return 256.0D < d0 * d0 + d1 * d1 + d2 * d2 ? null : (1 < i ? null : this.mc.effectRenderer.spawnEffectParticle(particleID, xCoord, yCoord, zCoord, xOffset, yOffset, zOffset, parameters));
            }
        } else {
            return null;
        }
    ...
    public void broadcastSound(int soundID, BlockPos pos, int data) {
        switch (soundID) {
            case 1013:
            case 1018:
                if (null != mc.getRenderViewEntity()) {
        ...
        switch (sfxType) {
            ...
            case 2002:
                ...
                for (int i1 = 0; 8 > i1; ++i1) {
                    this.spawnParticle(EnumParticleTypes.ITEM_CRACK, d13, d14, d16, random.nextGaussian() * 0.15D, random.nextDouble() * 0.2D, random.nextGaussian() * 0.15D, Item.getIdFromItem(Items.potionitem), data);
                }
            ...
            case 2003:
                ...
                for (double d18 = 0.0D; (Math.PI * 2D) > d18; d18 += 0.15707963267948966D) {
            ...
            case 2004:
                for (int k = 0; 20 > k; ++k) {
                    ...
                    this.theWorld.spawnParticle(EnumParticleTypes.FLAME, d3, d5, d7, 0.0D, 0.0D, 0.0D);
                }
    ...
    public void sendBlockBreakProgress(int breakerId, BlockPos pos, int progress) {
        if (0 <= progress && 10 > progress) {
            DestroyBlockProgress destroyblockprogress = this.damagedBlocks.get(Integer.valueOf(breakerId));

            if (null == destroyblockprogress || destroyblockprogress.getPosition().getX() != pos.getX() || destroyblockprogress.getPosition().getY() != pos.getY() || destroyblockprogress.getPosition().getZ() != pos.getZ()) {
