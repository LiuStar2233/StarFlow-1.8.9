...
public class RenderChunk {
    ...
    public void resortTransparency(float x, float y, float z, ChunkCompileTaskGenerator generator) {
        ...
        if (null != compiledchunk.getState() && !compiledchunk.isLayerEmpty(EnumWorldBlockLayer.TRANSLUCENT)) {
        ...
        if (!iblockaccess.extendedLevelsInChunkCache()) {
            ...
            for (BlockPos.MutableBlockPos blockpos$mutableblockpos : BlockPos.getAllInBoxMutable(blockpos, blockpos1)) {
                ...
                if (block.hasTileEntity()) {
                    ...
                    if (null != tileentity && null != tileentityspecialrenderer) {
    ...
    protected void finishCompileTask() {
        ...
        try {
            if (null != compileTask && ChunkCompileTaskGenerator.Status.DONE != compileTask.getStatus()) {
    ...
    public ChunkCompileTaskGenerator makeCompileTaskTransparency() {
        ...
        try {
            if (null == compileTask || ChunkCompileTaskGenerator.Status.PENDING != compileTask.getStatus()) {
                if (null != compileTask && ChunkCompileTaskGenerator.Status.DONE != compileTask.getStatus()) {
                    this.compileTask.finish();
                    this.compileTask = null;
                }
    ...
    private void postRenderBlocks(EnumWorldBlockLayer layer, float x, float y, float z, WorldRenderer worldRendererIn, CompiledChunk compiledChunkIn) {
        if (EnumWorldBlockLayer.TRANSLUCENT == layer && !compiledChunkIn.isLayerEmpty(layer)) {
