...
public class TextureAtlasSprite {
    ...
    public void updateAnimation() {
        ...
        if (this.tickCounter >= this.animationMetadata.getFrameTimeSingle(this.frameCounter)) {
            ...
            int j = 0 == animationMetadata.getFrameCount() ? this.framesTextureData.size() : this.animationMetadata.getFrameCount();
            this.frameCounter = (this.frameCounter + 1) % j;
            this.tickCounter = 0;
            int k = this.animationMetadata.getFrameIndex(this.frameCounter);

            if (i != k && 0 <= k && k < this.framesTextureData.size()) {
    ...
    private void updateAnimationInterpolated() {
        ...
        if (i != k && 0 <= k && k < this.framesTextureData.size()) {
            ...
            if (null == interpolatedFrameData || this.interpolatedFrameData.length != aint.length) {
                this.interpolatedFrameData = new int[aint.length][];
            }

            for (int l = 0; l < aint.length; ++l) {
                if (null == interpolatedFrameData[l]) {
    ...
    public void loadSprite(BufferedImage[] images, AnimationMetadataSection meta) throws IOException {
        ...
        for (int k = 0; k < images.length; ++k) {
            ...
            if (null != bufferedimage) {
                if (0 < k && (bufferedimage.getWidth() != i >> k || bufferedimage.getHeight() != j >> k)) {
                    ...
                    crashreportcategory.addCrashSectionCallable("Frame sizes", new Callable<String>() {
                        public String call() throws Exception {
                            ...
                            for (int[] aint1 : aint) {
                                ...
                                stringbuilder.append(null == aint1 ? "null" : Integer.valueOf(aint1.length));
                            }
