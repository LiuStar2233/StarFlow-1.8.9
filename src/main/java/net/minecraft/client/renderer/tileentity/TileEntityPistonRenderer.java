...
public class TileEntityPistonRenderer extends TileEntitySpecialRenderer<TileEntityPiston> {
    ...
    public void renderTileEntityAt(TileEntityPiston te, double x, double y, double z, float partialTicks, int destroyStage) {
        ...
        if (block.getMaterial() != Material.air && 1.0F > te.getProgress(partialTicks)) {
            ...
            if (block == Blocks.piston_head && 0.5F > te.getProgress(partialTicks)) {
                ...
                iblockstate1 = iblockstate1.withProperty(BlockPistonExtension.SHORT, Boolean.valueOf(0.5F <= te.getProgress(partialTicks)));
