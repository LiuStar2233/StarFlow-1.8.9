...
public class ModelBakery {
    ...
    private void loadVariantModels() {
        for (ModelResourceLocation modelresourcelocation : this.variants.keySet()) {
            for (ModelBlockDefinition.Variant modelblockdefinition$variant : this.variants.get(modelresourcelocation).getVariants()) {
                ...
                if (null == models.get(resourcelocation)) {
                    try {
    ...
    private void bakeBlockModels() {
        for (ModelResourceLocation modelresourcelocation : this.variants.keySet()) {
            ...
            for (ModelBlockDefinition.Variant modelblockdefinition$variant : this.variants.get(modelresourcelocation).getVariants()) {
                ...
                if (null != modelblock && modelblock.isResolved()) {
                    ++i;
        ...
        for (final Map.Entry<String, ResourceLocation> entry : itemLocations.entrySet()) {
            final ResourceLocation resourcelocation = entry.getValue();
            final ModelResourceLocation modelresourcelocation1 = new ModelResourceLocation(entry.getKey(), "inventory");
            final ModelBlock modelblock1 = models.get(resourcelocation);

            if (null != modelblock1 && modelblock1.isResolved()) {
    ...
    private Set<ResourceLocation> getVariantsTextureLocations() {
        ...
        final List<ModelResourceLocation> list = Lists.newArrayList(variants.keySet());
        ...
        for (final ModelResourceLocation modelresourcelocation : list) {
            final ModelBlockDefinition.Variants modelblockdefinition$variants = variants.get(modelresourcelocation);

            for (final ModelBlockDefinition.Variant modelblockdefinition$variant : modelblockdefinition$variants.getVariants()) {
                final ModelBlock modelblock = models.get(modelblockdefinition$variant.getModelLocation());
    ...
    private IBakedModel bakeModel(final ModelBlock modelBlockIn, final ModelRotation modelRotationIn, final boolean uvLocked) {
        final TextureAtlasSprite textureatlassprite = sprites.get(new ResourceLocation(modelBlockIn.resolveTextureName("particle")));
        ...
        for (final BlockPart blockpart : modelBlockIn.getElements()) {
            for (final EnumFacing enumfacing : blockpart.mapFaces.keySet()) {
                ...
                final TextureAtlasSprite textureatlassprite1 = sprites.get(new ResourceLocation(modelBlockIn.resolveTextureName(blockpartface.texture)));

                if (null == blockpartface.cullFace) {
    ...
    private void loadModelsCheck() {
        ...
        for (final ModelBlock modelblock : models.values()) {
    ...
    private void loadModels() {
        ...
        for (final ResourceLocation resourcelocation : models.keySet()) {
            set.add(resourcelocation);
            final ResourceLocation resourcelocation1 = models.get(resourcelocation).getParentLocation();

            if (null != resourcelocation1) {
        ...
        while (!deque.isEmpty()) {
            ...
            try {
                if (null != this.models.get(resourcelocation2)) {
                    continue;
                }

                final ModelBlock modelblock = loadModel(resourcelocation2);
                models.put(resourcelocation2, modelblock);
                final ResourceLocation resourcelocation3 = modelblock.getParentLocation();

                if (null != resourcelocation3 && !set.contains(resourcelocation3)) {
                    deque.add(resourcelocation3);
                }
            } catch (final Exception exception) {
                ModelBakery.LOGGER.warn("In parent chain: " + ModelBakery.JOINER.join(getParentPath(resourcelocation2)) + "; unable to load model: '" + resourcelocation2 + "'", exception);
            }
    ...
    private List<ResourceLocation> getParentPath(final ResourceLocation p_177573_1_) {
        ...
        while (null != (resourcelocation = this.getParentLocation(resourcelocation))) {
    ...
    private ResourceLocation getParentLocation(final ResourceLocation p_177576_1_) {
        for (Map.Entry<ResourceLocation, ModelBlock> entry : this.models.entrySet()) {
            ...
            if (null != modelblock && p_177576_1_.equals(modelblock.getParentLocation())) {
                return entry.getKey();
            }
        }
