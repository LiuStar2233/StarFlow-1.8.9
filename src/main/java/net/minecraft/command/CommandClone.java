...
public class CommandClone extends CommandBase {
    ...
    public void processCommand(ICommandSender sender, String[] args) throws CommandException {
        if (9 > args.length) {
            ...
            if (32768 < i) {
                ...
                if ((11 > args.length || !"force".equals(args[10]) && !"move".equals(args[10])) && structureboundingbox.intersectsWith(structureboundingbox1)) {
                    throw new CommandException("commands.clone.noOverlap");
                } else {
                    if (11 <= args.length && "move".equals(args[10])) {
                        flag = true;
                    }

                    if (0 <= structureboundingbox.minY && 256 > structureboundingbox.maxY && 0 <= structureboundingbox1.minY && 256 > structureboundingbox1.maxY) {
                        ...
                        if (world.isAreaLoaded(structureboundingbox) && world.isAreaLoaded(structureboundingbox1)) {
                            ...
                            for (int k = structureboundingbox.minZ; k <= structureboundingbox.maxZ; ++k) {
                                for (int l = structureboundingbox.minY; l <= structureboundingbox.maxY; ++l) {
                                    for (int i1 = structureboundingbox.minX; i1 <= structureboundingbox.maxX; ++i1) {
                                        ...
                                        if ((!flag1 || iblockstate.getBlock() != Blocks.air) && (null == block || iblockstate.getBlock() == block && (0 > j || iblockstate.getBlock().getMetaFromState(iblockstate) == j))) {
                            ...
                            for (CommandClone.StaticCloneData commandclone$staticclonedata2 : list1) {
                                ...
                                if (null != commandclone$staticclonedata2.compound && null != tileentity3) {
    ...
    public List<String> addTabCompletionOptions(ICommandSender sender, String[] args, BlockPos pos) {
        return 0 < args.length && 3 >= args.length ? func_175771_a(args, 0, pos) : (3 < args.length && 6 >= args.length ? func_175771_a(args, 3, pos) : (6 < args.length && 9 >= args.length ? func_175771_a(args, 6, pos) : (10 == args.length ? getListOfStringsMatchingLastWord(args, "replace", "masked", "filtered") : (11 == args.length ? getListOfStringsMatchingLastWord(args, "normal", "force", "move") : (12 == args.length && "filtered".equals(args[9]) ? getListOfStringsMatchingLastWord(args, Block.blockRegistry.getKeys()) : null)))));
    }
