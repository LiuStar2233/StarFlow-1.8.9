...
public class CommandCompare extends CommandBase {
    ...
    public void processCommand(ICommandSender sender, String[] args) throws CommandException {
        if (9 > args.length) {
            ...
            if (524288 < i) {
                throw new CommandException("commands.compare.tooManyBlocks", Integer.valueOf(i), Integer.valueOf(524288));
            } else if (0 <= structureboundingbox.minY && 256 > structureboundingbox.maxY && 0 <= structureboundingbox1.minY && 256 > structureboundingbox1.maxY) {
                World world = sender.getEntityWorld();

                if (world.isAreaLoaded(structureboundingbox) && world.isAreaLoaded(structureboundingbox1)) {
                    boolean flag = 9 < args.length && "masked".equals(args[9]);
                            ...
                            for (int l = structureboundingbox.minX; l <= structureboundingbox.maxX; ++l) {
                                ...
                                if (!flag || iblockstate.getBlock() != Blocks.air) {
                                    if (iblockstate == world.getBlockState(blockpos$mutableblockpos1)) {
                                        ...
                                        if (null != tileentity && null != tileentity1) {
                    ...
                    notifyOperators(sender, this, "commands.compare.success", Integer.valueOf(i));
                } else {
    ...
    public List<String> addTabCompletionOptions(ICommandSender sender, String[] args, BlockPos pos) {
        return 0 < args.length && 3 >= args.length ? func_175771_a(args, 0, pos) : (3 < args.length && 6 >= args.length ? func_175771_a(args, 3, pos) : (6 < args.length && 9 >= args.length ? func_175771_a(args, 6, pos) : (10 == args.length ? getListOfStringsMatchingLastWord(args, "masked", "all") : null)));
    }
}