...
public class CommandExecuteAt extends CommandBase {
    ...
    public void processCommand(final ICommandSender sender, String[] args) throws CommandException {
        if (5 > args.length) {
            ...
            if ("detect".equals(args[4]) && 10 < args.length) {
                ...
                if (iblockstate.getBlock() != block || 0 <= k && iblockstate.getBlock().getMetaFromState(iblockstate) != k) {
            ...
            ICommandSender icommandsender = new ICommandSender() {
                ...
                public boolean sendCommandFeedback() {
                    ...
                    return null == minecraftserver || minecraftserver.worldServers[0].getGameRules().getBoolean("commandBlockOutput");
                }
    ...
    public List<String> addTabCompletionOptions(ICommandSender sender, String[] args, BlockPos pos) {
        return 1 == args.length ? getListOfStringsMatchingLastWord(args, MinecraftServer.getServer().getAllUsernames()) : (1 < args.length && 4 >= args.length ? func_175771_a(args, 1, pos) : (5 < args.length && 8 >= args.length && "detect".equals(args[4]) ? func_175771_a(args, 5, pos) : (9 == args.length && "detect".equals(args[4]) ? getListOfStringsMatchingLastWord(args, Block.blockRegistry.getKeys()) : null)));
    }
