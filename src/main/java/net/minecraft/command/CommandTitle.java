...
public class CommandTitle extends CommandBase {
    ...
    public void processCommand(ICommandSender sender, String[] args) throws CommandException {
        if (2 > args.length) {
            ...
            if (S45PacketTitle.Type.CLEAR != s45packettitle$type && S45PacketTitle.Type.RESET != s45packettitle$type) {
                if (S45PacketTitle.Type.TIMES == s45packettitle$type) {
                    if (5 != args.length) {
                        throw new WrongUsageException("commands.title.usage");
                    } else {
                    ...
                    try {
                        ...
                        throw new SyntaxErrorException("commands.tellraw.jsonException", null == throwable ? "" : throwable.getMessage());
                    }
    ...
    public List<String> addTabCompletionOptions(ICommandSender sender, String[] args, BlockPos pos) {
        return 1 == args.length ? getListOfStringsMatchingLastWord(args, MinecraftServer.getServer().getAllUsernames()) : (2 == args.length ? getListOfStringsMatchingLastWord(args, S45PacketTitle.Type.getNames()) : null);
    }
