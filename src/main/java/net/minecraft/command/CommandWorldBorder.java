...
public class CommandWorldBorder extends CommandBase {
    ...
    public void processCommand(ICommandSender sender, String[] args) throws CommandException {
        if (1 > args.length) {
            ...
            if ("set".equals(args[0])) {
                ...
                long i = 2 < args.length ? parseLong(args[2], 0L, 9223372036854775L) * 1000L : 0L;
                ...
                long i1 = worldborder.getTimeUntilTarget() + (2 < args.length ? parseLong(args[2], 0L, 9223372036854775L) * 1000L : 0L);
    ...
    public List<String> addTabCompletionOptions(ICommandSender sender, String[] args, BlockPos pos) {
        return 1 == args.length ? getListOfStringsMatchingLastWord(args, "set", "center", "damage", "warning", "add", "get") : (2 == args.length && "damage".equals(args[0]) ? getListOfStringsMatchingLastWord(args, "buffer", "amount") : (2 <= args.length && 3 >= args.length && "center".equals(args[0]) ? func_181043_b(args, 1, pos) : (2 == args.length && "warning".equals(args[0]) ? getListOfStringsMatchingLastWord(args, "time", "distance") : null)));
    }
}