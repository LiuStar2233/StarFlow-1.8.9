...
public class CommandAchievement extends CommandBase {
    ...
    public void processCommand(ICommandSender sender, String[] args) throws CommandException {
        if (2 > args.length) {
            ...
            if (null == statbase && !"*".equals(args[1])) {
                throw new CommandException("commands.achievement.unknownAchievement", args[1]);
            } else {
                final EntityPlayerMP entityplayermp = 3 <= args.length ? getPlayer(sender, args[2]) : getCommandSenderAsPlayer(sender);
                    ...
                    if (null == statbase) {
                        ...
                        if (statbase instanceof Achievement) {
                            ...
                            if (flag) {
                                ...
                                for (list = Lists.newArrayList(); null != achievement.parentAchievement && !entityplayermp.getStatFile().hasAchievementUnlocked(achievement.parentAchievement); achievement = achievement.parentAchievement) {
                                ...
                                for (Achievement achievement2 : list1) {
                                    ...
                                    for (flag2 = false; null != achievement3; achievement3 = achievement3.parentAchievement) {
                                    ...
                                    if (!flag2) {
                                        for (achievement3 = achievement2; null != achievement3; achievement3 = achievement3.parentAchievement) {
    ...
    public List<String> addTabCompletionOptions(ICommandSender sender, String[] args, BlockPos pos) {
        if (1 == args.length) {
            ...
            return 3 == args.length ? getListOfStringsMatchingLastWord(args, MinecraftServer.getServer().getAllUsernames()) : null;
        } else {
