...
public class EnchantmentDamage extends Enchantment {
    ...
    public float calcDamageByCreature(int level, EnumCreatureAttribute creatureType) {
        return 0 == damageType ? (float) level * 1.25F : (1 == damageType && EnumCreatureAttribute.UNDEAD == creatureType ? (float) level * 2.5F : (2 == damageType && EnumCreatureAttribute.ARTHROPOD == creatureType ? (float) level * 2.5F : 0.0F));
    }
    ...
    public void onEntityDamaged(EntityLivingBase user, Entity target, int level) {
        if (target instanceof EntityLivingBase) {
            ...
            if (2 == damageType && EnumCreatureAttribute.ARTHROPOD == entitylivingbase.getCreatureAttribute()) {
