...
public class EnchantmentProtection extends Enchantment {
    ...
    public int calcModifierDamage(int level, DamageSource source) {
        if (source.canHarmInCreative()) {
            ...
            return 0 == protectionType ? MathHelper.floor_float(f * 0.75F) : (1 == protectionType && source.isFireDamage() ? MathHelper.floor_float(f * 1.25F) : (2 == protectionType && source == DamageSource.fall ? MathHelper.floor_float(f * 2.5F) : (3 == protectionType && source.isExplosion() ? MathHelper.floor_float(f * 1.5F) : (4 == protectionType && source.isProjectile() ? MathHelper.floor_float(f * 1.5F) : 0))));
        }
    }
    ...
    public boolean canApplyTogether(Enchantment ench) {
        if (ench instanceof EnchantmentProtection) {
            ...
            return enchantmentprotection.protectionType != this.protectionType && (2 == protectionType || 2 == enchantmentprotection.protectionType);
        } else {
