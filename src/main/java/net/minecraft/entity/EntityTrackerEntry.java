...
public class EntityTrackerEntry {
    ...
    public void updatePlayerList(List<EntityPlayer> players) {
        ...
        if (!this.firstUpdateDone || 16.0D < trackedEntity.getDistanceSq(lastTrackedEntityPosX, lastTrackedEntityPosY, lastTrackedEntityPosZ)) {
        ...
        if (this.field_85178_v != this.trackedEntity.ridingEntity || null != trackedEntity.ridingEntity && 0 == updateCounter % 60) {
        ...
        if (this.trackedEntity instanceof EntityItemFrame && 0 == updateCounter % 10) {
            EntityItemFrame entityitemframe = (EntityItemFrame) this.trackedEntity;
            ItemStack itemstack = entityitemframe.getDisplayedItem();

            if (null != itemstack && itemstack.getItem() instanceof ItemMap) {
        ...
        if (0 == updateCounter % updateFrequency || this.trackedEntity.isAirBorne || this.trackedEntity.getDataWatcher().hasObjectChanged()) {
            if (null == trackedEntity.ridingEntity) {
                ...
                Packet packet1 = null;
                boolean flag = 4 <= Math.abs(j2) || 4 <= Math.abs(k2) || 4 <= Math.abs(i) || 0 == updateCounter % 60;
                boolean flag1 = 4 <= Math.abs(l1 - encodedRotationYaw) || 4 <= Math.abs(i2 - encodedRotationPitch);

                if (0 < updateCounter || this.trackedEntity instanceof EntityArrow) {
                    if (-128 <= j2 && 128 > j2 && -128 <= k2 && 128 > k2 && -128 <= i && 128 > i && 400 >= ticksSinceLastForcedTeleport && !this.ridingEntity && this.onGround == this.trackedEntity.onGround) {
                ...
                if (this.sendVelocityUpdates) {
                    ...
                    if (d3 * d3 < d4 || 0.0D < d4 && 0.0D == trackedEntity.motionX && 0.0D == trackedEntity.motionY && 0.0D == trackedEntity.motionZ) {
                ...
                boolean flag2 = 4 <= Math.abs(j - encodedRotationYaw) || 4 <= Math.abs(i1 - encodedRotationPitch);
            ...
            if (4 <= Math.abs(l - lastHeadMotion)) {
        ...
        if (playerMP != this.trackedEntity) {
            if (this.func_180233_c(playerMP)) {
                if (!this.trackingPlayers.contains(playerMP) && (this.isPlayerWatchingThisChunk(playerMP) || this.trackedEntity.forceSpawn)) {
                    ...
                    if (this.trackedEntity instanceof EntityLiving && null != ((EntityLiving) trackedEntity).getLeashedToEntity()) {
    ...
    private Packet createSpawnPacket() {
        ...
        if (this.trackedEntity instanceof EntityItem) {
            ...
            return new S0EPacketSpawnObject(this.trackedEntity, 90, null != entity1 ? entity1.getEntityId() : this.trackedEntity.getEntityId());
        } else if (this.trackedEntity instanceof EntityArrow) {
            Entity entity = ((EntityArrow) this.trackedEntity).shootingEntity;
            return new S0EPacketSpawnObject(this.trackedEntity, 60, null != entity ? entity.getEntityId() : this.trackedEntity.getEntityId());
            ...
            if (null != entityfireball.shootingEntity) {
