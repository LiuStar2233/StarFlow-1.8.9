...
public abstract class EntityAIDoorInteract extends EntityAIBase {
    ...
    public boolean shouldExecute() {
        if (!this.theEntity.isCollidedHorizontally) {
            ...
            if (null != pathentity && !pathentity.isFinished() && pathnavigateground.getEnterDoors()) {
                for (int i = 0; i < Math.min(pathentity.getCurrentPathIndex() + 2, pathentity.getCurrentPathLength()); ++i) {
                    PathPoint pathpoint = pathentity.getPathPointFromIndex(i);
                    this.doorPosition = new BlockPos(pathpoint.xCoord, pathpoint.yCoord + 1, pathpoint.zCoord);

                    if (2.25D >= theEntity.getDistanceSq(doorPosition.getX(), theEntity.posY, doorPosition.getZ())) {
