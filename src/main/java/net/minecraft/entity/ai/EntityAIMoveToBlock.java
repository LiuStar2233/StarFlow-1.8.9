...
public abstract class EntityAIMoveToBlock extends EntityAIBase {
    ...
    public boolean continueExecuting() {
        return this.timeoutCounter >= -this.field_179490_f && 1200 >= timeoutCounter && this.shouldMoveTo(this.theEntity.worldObj, this.destinationBlock);
    }
    ...
    public void updateTask() {
        if (1.0D < theEntity.getDistanceSqToCenter(destinationBlock.up())) {
            this.isAboveDestination = false;
            ++this.timeoutCounter;

            if (0 == timeoutCounter % 40) {
                this.theEntity.getNavigator().tryMoveToXYZ((double) ((float) this.destinationBlock.getX()) + 0.5D, this.destinationBlock.getY() + 1, (double) ((float) this.destinationBlock.getZ()) + 0.5D, this.movementSpeed);
            }
        } else {
    ...
    private boolean searchForDestination() {
        ...
        for (int k = 0; 1 >= k; k = 0 < k ? -k : 1 - k) {
            for (int l = 0; l < i; ++l) {
                for (int i1 = 0; i1 <= l; i1 = 0 < i1 ? -i1 : 1 - i1) {
                    for (int j1 = i1 < l && i1 > -l ? l : 0; j1 <= l; j1 = 0 < j1 ? -j1 : 1 - j1) {
