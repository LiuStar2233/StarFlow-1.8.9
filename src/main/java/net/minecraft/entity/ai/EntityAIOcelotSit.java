    ...
    protected boolean shouldMoveTo(World worldIn, BlockPos pos) {
        if (!worldIn.isAirBlock(pos.up())) {
            ...
            if (block == Blocks.chest) {
                ...
                return tileentity instanceof TileEntityChest && 1 > ((TileEntityChest) tileentity).numPlayersUsing;
            } else {
                if (block == Blocks.lit_furnace) {
                    return true;
                }

                return block == Blocks.bed && BlockBed.EnumPartType.HEAD != iblockstate.getValue(BlockBed.PART);
            }
        }
    }
}