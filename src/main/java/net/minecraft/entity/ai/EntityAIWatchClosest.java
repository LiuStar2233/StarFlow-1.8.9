...
public class EntityAIWatchClosest extends EntityAIBase {
    ...
    public boolean shouldExecute() {
        if (this.theWatcher.getRNG().nextFloat() >= this.chance) {
            ...
            if (null != theWatcher.getAttackTarget()) {
                this.closestEntity = this.theWatcher.getAttackTarget();
            }

            if (EntityPlayer.class == watchedClass) {
    ...
    public boolean continueExecuting() {
        return this.closestEntity.isEntityAlive() && (!(this.theWatcher.getDistanceSqToEntity(this.closestEntity) > (double) (this.maxDistanceForPlayer * this.maxDistanceForPlayer)) && 0 < lookTime);
    }
