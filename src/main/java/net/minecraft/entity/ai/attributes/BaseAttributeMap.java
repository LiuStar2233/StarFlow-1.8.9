...
public abstract class BaseAttributeMap {
    ...
    public IAttributeInstance registerAttribute(IAttribute attribute) {
        if (this.attributesByName.containsKey(attribute.getAttributeUnlocalizedName())) {
            ...
            for (IAttribute iattribute = attribute.func_180372_d(); null != iattribute; iattribute = iattribute.func_180372_d()) {
    ...
    public void removeAttributeModifiers(Multimap<String, AttributeModifier> modifiers) {
        for (final Map.Entry<String, AttributeModifier> entry : modifiers.entries()) {
            final IAttributeInstance iattributeinstance = getAttributeInstanceByName(entry.getKey());
