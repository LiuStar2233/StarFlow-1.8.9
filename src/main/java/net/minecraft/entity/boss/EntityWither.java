...
public class EntityWither extends EntityMob implements IBossDisplayData, IRangedAttackMob {
    ...
    private static final Predicate<Entity> attackEntitySelector = new Predicate<Entity>() {
        public boolean apply(Entity p_apply_1_) {
            return p_apply_1_ instanceof EntityLivingBase && EnumCreatureAttribute.UNDEAD != ((EntityLivingBase) p_apply_1_).getCreatureAttribute();
        }
    };
    ...
    public void onLivingUpdate() {
        ...
        if (!this.worldObj.isRemote && 0 < getWatchedTargetId(0)) {
        ...
        if (0.05000000074505806D < motionX * motionX + motionZ * motionZ) {
        ...
        for (int l = 0; 3 > l; ++l) {
            ...
            if (flag && 0 == worldObj.rand.nextInt(4)) {
        ...
        if (0 < getInvulTime()) {
            ...
            for (int i = 1; 3 > i; ++i) {
                if (this.ticksExisted >= this.field_82223_h[i - 1]) {
                    ...
                    if (EnumDifficulty.NORMAL == worldObj.getDifficulty() || EnumDifficulty.HARD == worldObj.getDifficulty()) {
                        int j3 = i - 1;
                    ...
                    if (0 < k1) {
                        ...
                        if (null != entity && entity.isEntityAlive() && 900.0D >= getDistanceSqToEntity(entity) && this.canEntityBeSeen(entity)) {
                        ...
                        for (int j2 = 0; 10 > j2 && !list.isEmpty(); ++j2) {
            ...
            if (0 < blockBreakCounter) {
                --this.blockBreakCounter;

                if (0 == blockBreakCounter && this.worldObj.getGameRules().getBoolean("mobGriefing")) {
                    int i1 = MathHelper.floor_double(this.posY);
                    int l1 = MathHelper.floor_double(this.posX);
                    int i2 = MathHelper.floor_double(this.posZ);
                    boolean flag = false;

                    for (int k2 = -1; 1 >= k2; ++k2) {
                        for (int l2 = -1; 1 >= l2; ++l2) {
                            for (int j = 0; 3 >= j; ++j) {
                                int i3 = l1 + k2;
                                int k = i1 + j;
                                int l = i2 + l2;
    ...
    private double func_82208_v(int p_82208_1_) {
        return 0 >= p_82208_1_ ? this.posY + 3.0D : this.posY + 2.2D;
    }
    ...
    private void launchWitherSkullToEntity(int p_82216_1_, EntityLivingBase p_82216_2_) {
        this.launchWitherSkullToCoords(p_82216_1_, p_82216_2_.posX, p_82216_2_.posY + (double) p_82216_2_.getEyeHeight() * 0.5D, p_82216_2_.posZ, 0 == p_82216_1_ && 0.001F > rand.nextFloat());
    }
    ...
    public boolean attackEntityFrom(DamageSource source, float amount) {
        if (this.isEntityInvulnerable(source)) {
            ...
            if (0 < getInvulTime() && source != DamageSource.outOfWorld) {
                return false;
            } else {
                if (this.isArmored()) {
                ...
                if (null != entity1 && !(entity1 instanceof EntityPlayer) && entity1 instanceof EntityLivingBase && ((EntityLivingBase) entity1).getCreatureAttribute() == this.getCreatureAttribute()) {
                    return false;
                } else {
                    if (0 >= blockBreakCounter) {
