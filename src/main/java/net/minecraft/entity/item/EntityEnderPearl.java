...
public class EntityEnderPearl extends EntityThrowable {
        ...
        if (!this.worldObj.isRemote) {
            if (entitylivingbase instanceof EntityPlayerMP) {
                ...
                if (entityplayermp.playerNetServerHandler.getNetworkManager().isChannelOpen() && entityplayermp.worldObj == this.worldObj && !entityplayermp.isPlayerSleeping()) {
                    if (0.05F > rand.nextFloat() && this.worldObj.getGameRules().getBoolean("doMobSpawning")) {
    ...
    public void onUpdate() {
        ...
        if (null != entitylivingbase && entitylivingbase instanceof EntityPlayer && !entitylivingbase.isEntityAlive()) {
            this.setDead();
        } else {
            super.onUpdate();
        }
