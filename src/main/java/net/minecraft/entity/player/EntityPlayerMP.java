...
public class EntityPlayerMP extends EntityPlayer implements ICrafting {
    ...
    public EntityPlayerMP(MinecraftServer server, WorldServer worldIn, GameProfile profile, ItemInWorldManager interactionManager) {
        ...
        if (!worldIn.provider.getHasNoSky() && WorldSettings.GameType.ADVENTURE != worldIn.getWorldInfo().getGameType()) {
        ...
        while (!worldIn.getCollidingBoundingBoxes(this, this.getEntityBoundingBox()).isEmpty() && 255.0D > posY) {
    ...
    public void onUpdate() {
        ...
        if (!this.loadedChunks.isEmpty()) {
            ...
            while (iterator1.hasNext() && 10 > list.size()) {
                ChunkCoordIntPair chunkcoordintpair = iterator1.next();

                if (null != chunkcoordintpair) {
    ...
    public void onUpdateEntity() {
        try {
            ...
            for (int i = 0; i < this.inventory.getSizeInventory(); ++i) {
                ...
                if (null != itemstack && itemstack.getItem().isMap()) {
            ...
            if (this.getHealth() != this.lastHealth || this.lastFoodLevel != this.foodStats.getFoodLevel() || 0.0F == foodStats.getSaturationLevel() != this.wasHungry) {
                ...
                this.wasHungry = 0.0F == foodStats.getSaturationLevel();
            }
            ...
            if (this.getHealth() + this.getAbsorptionAmount() != this.combinedHealth) {
                ...
                for (ScoreObjective scoreobjective : this.getWorldScoreboard().getObjectivesFromCriteria(IScoreObjectiveCriteria.health)) {
                    this.getWorldScoreboard().getValueFromObjective(this.getName(), scoreobjective).func_96651_a(Collections.singletonList(this));
                }
            }
            ...
            if (0 == ticksExisted % 20 * 5 && !this.getStatFile().hasAchievementUnlocked(AchievementList.exploreAllBiomes)) {
    ...
    public void onDeath(DamageSource cause) {
        if (this.worldObj.getGameRules().getBoolean("showDeathMessages")) {
            ...
            if (null != team && Team.EnumVisible.ALWAYS != team.getDeathMessageVisibility()) {
                if (Team.EnumVisible.HIDE_FOR_OTHER_TEAMS == team.getDeathMessageVisibility()) {
                    this.mcServer.getConfigurationManager().sendMessageToAllTeamMembers(this, this.getCombatTracker().getDeathMessage());
                } else if (Team.EnumVisible.HIDE_FOR_OWN_TEAM == team.getDeathMessageVisibility()) {
    ...
    public boolean attackEntityFrom(DamageSource source, float amount) {
        if (this.isEntityInvulnerable(source)) {
            ...
            if (!flag && 0 < respawnInvulnerabilityTicks && source != DamageSource.outOfWorld) {
                return false;
            } else {
    ...
    public EntityPlayer.EnumStatus trySleep(BlockPos bedLocation) {
        ...
        if (EnumStatus.OK == entityplayer$enumstatus) {
    ...
    public void wakeUpPlayer(boolean immediately, boolean updateWorldFlag, boolean setSpawn) {
        ...
        if (null != playerNetServerHandler) {
            this.playerNetServerHandler.setPlayerLocation(this.posX, this.posY, this.posZ, this.rotationYaw, this.rotationPitch);
        }
    }
    ...
    public void setEntityActionState(float p_110430_1_, float p_110430_2_, boolean p_110430_3_, boolean sneaking) {
        if (null != ridingEntity) {
            if (-1.0F <= p_110430_1_ && 1.0F >= p_110430_1_) {
                this.moveStrafing = p_110430_1_;
            }

            if (-1.0F <= p_110430_2_ && 1.0F >= p_110430_2_) {
    ...
    public void setItemInUse(ItemStack stack, int duration) {
        ...
        if (null != stack && null != stack.getItem() && EnumAction.EAT == stack.getItem().getItemUseAction(stack)) {
    ...
    public void setGameType(WorldSettings.GameType gameType) {
        ...
        if (WorldSettings.GameType.SPECTATOR == gameType) {
    ...
    public boolean isSpectator() {
        return WorldSettings.GameType.SPECTATOR == theItemInWorldManager.getGameType();
    }
    ...
    public boolean canCommandSenderUseCommand(int permLevel, String commandName) {
        if ("seed".equals(commandName) && !this.mcServer.isDedicatedServer()) {
            ...
            if (this.mcServer.getConfigurationManager().canSendCommands(this.getGameProfile())) {
                ...
                return null != userlistopsentry ? userlistopsentry.getPermissionLevel() >= permLevel : this.mcServer.getOpPermissionLevel() >= permLevel;
            } else {
                return false;
            }
        } else {
    ...
    public Entity getSpectatingEntity() {
        return null == spectatingEntity ? this : this.spectatingEntity;
    }

    public void setSpectatingEntity(Entity entityToSpectate) {
        Entity entity = this.getSpectatingEntity();
        this.spectatingEntity = null == entityToSpectate ? this : entityToSpectate;
    ...
    public void attackTargetEntityWithCurrentItem(Entity targetEntity) {
        if (WorldSettings.GameType.SPECTATOR == theItemInWorldManager.getGameType()) {
