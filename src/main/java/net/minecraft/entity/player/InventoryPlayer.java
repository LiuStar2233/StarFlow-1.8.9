...
public class InventoryPlayer implements IInventory {
    ...
    public ItemStack getCurrentItem() {
        return 9 > currentItem && 0 <= currentItem ? this.mainInventory[this.currentItem] : null;
    }
    ...
    private int getInventorySlotContainItem(Item itemIn) {
        for (int i = 0; i < this.mainInventory.length; ++i) {
            if (null != mainInventory[i] && this.mainInventory[i].getItem() == itemIn) {
                return i;
            }
        }
    ...
    private int getInventorySlotContainItemAndDamage(Item itemIn, int metadataIn) {
        for (int i = 0; i < this.mainInventory.length; ++i) {
            if (null != mainInventory[i] && this.mainInventory[i].getItem() == itemIn && this.mainInventory[i].getMetadata() == metadataIn) {
                return i;
            }
        }
    ...
    public void setCurrentItem(Item itemIn, int metadataIn, boolean isMetaSpecific, boolean p_146030_4_) {
        ...
        if (0 <= i && 9 > i) {
            this.currentItem = i;
        } else if (p_146030_4_ && null != itemIn) {
            ...
            if (null == itemstack || !itemstack.isItemEnchantable() || this.getInventorySlotContainItemAndDamage(itemstack.getItem(), itemstack.getItemDamage()) != this.currentItem) {
    ...
    public void changeCurrentItem(int direction) {
        ...
        for (this.currentItem -= direction; 0 > currentItem; this.currentItem += 9) {
        }
    ...
    public int clearMatchingItems(Item itemIn, int metadataIn, int removeCount, NBTTagCompound itemNBT) {
        ...
        for (int j = 0; j < this.mainInventory.length; ++j) {
            ...
            if (null != itemstack && (null == itemIn || itemstack.getItem() == itemIn) && (-1 >= metadataIn || itemstack.getMetadata() == metadataIn) && (null == itemNBT || NBTUtil.func_181123_a(itemNBT, itemstack.getTagCompound(), true))) {
                int k = 0 >= removeCount ? itemstack.stackSize : Math.min(removeCount - i, itemstack.stackSize);
                i += k;

                if (0 != removeCount) {
                    this.mainInventory[j].stackSize -= k;

                    if (0 == mainInventory[j].stackSize) {
                        this.mainInventory[j] = null;
                    }

                    if (0 < removeCount && i >= removeCount) {
                        return i;
                    }
                }
            }
        ...
        for (int l = 0; l < this.armorInventory.length; ++l) {
            ...
            if (null != itemstack1 && (null == itemIn || itemstack1.getItem() == itemIn) && (-1 >= metadataIn || itemstack1.getMetadata() == metadataIn) && (null == itemNBT || NBTUtil.func_181123_a(itemNBT, itemstack1.getTagCompound(), false))) {
                int j1 = 0 >= removeCount ? itemstack1.stackSize : Math.min(removeCount - i, itemstack1.stackSize);
                i += j1;

                if (0 != removeCount) {
                    this.armorInventory[l].stackSize -= j1;

                    if (0 == armorInventory[l].stackSize) {
        ...
        if (null != itemStack) {
            if (null != itemIn && this.itemStack.getItem() != itemIn) {
                return i;
            }

            if (-1 < metadataIn && this.itemStack.getMetadata() != metadataIn) {
                return i;
            }

            if (null != itemNBT && !NBTUtil.func_181123_a(itemNBT, this.itemStack.getTagCompound(), false)) {
                return i;
            }

            int i1 = 0 >= removeCount ? this.itemStack.stackSize : Math.min(removeCount - i, this.itemStack.stackSize);
            i += i1;
    ...
    public boolean consumeInventoryItem(Item itemIn) {
        ...
        if (0 > i) {
            ...
            if (0 >= --mainInventory[i].stackSize) {
    ...
    public boolean addItemStackToInventory(final ItemStack itemStackIn) {
        if (null != itemStackIn && 0 != itemStackIn.stackSize && null != itemStackIn.getItem()) {
            try {
                if (itemStackIn.isItemDamaged()) {
                    ...
                    while (true) {
                        ...
                        if (0 >= itemStackIn.stackSize || itemStackIn.stackSize >= i) {
                            break;
                        }
                    }
    ...
    public ItemStack decrStackSize(int index, int count) {
        ...
        if (null != aitemstack[index]) {
            if (aitemstack[index].stackSize <= count) {
                ...
                if (0 == aitemstack[index].stackSize) {
                    aitemstack[index] = null;
                }
    ...
    public float getStrVsBlock(Block blockIn) {
        ...
        if (null != mainInventory[currentItem]) {
    ...
    public void readFromNBT(NBTTagList nbtTagListIn) {
        ...
        for (int i = 0; i < nbtTagListIn.tagCount(); ++i) {
            ...
            if (null != itemstack) {
                if (0 <= j && j < this.mainInventory.length) {
                    this.mainInventory[j] = itemstack;
                }

                if (100 <= j && j < this.armorInventory.length + 100) {
    ...
    public boolean canHeldItemHarvest(Block blockIn) {
        if (blockIn.getMaterial().isToolNotRequired()) {
            ...
            return null != itemstack && itemstack.canHarvestBlock(blockIn);
        }
    }
    ...
    public int getTotalArmorValue() {
        ...
        for (int j = 0; j < this.armorInventory.length; ++j) {
            if (null != armorInventory[j] && this.armorInventory[j].getItem() instanceof ItemArmor) {
    ...
    public void damageArmor(float damage) {
        ...
        for (int i = 0; i < this.armorInventory.length; ++i) {
            if (null != armorInventory[i] && this.armorInventory[i].getItem() instanceof ItemArmor) {
                this.armorInventory[i].damageItem((int) damage, this.player);

                if (0 == armorInventory[i].stackSize) {
    ...
    public boolean isUseableByPlayer(EntityPlayer player) {
        return !this.player.isDead && 64.0D >= player.getDistanceSqToEntity(this.player);
    }
    ...
    public boolean hasItemStack(ItemStack itemStackIn) {
        for (int i = 0; i < this.armorInventory.length; ++i) {
            if (null != armorInventory[i] && this.armorInventory[i].isItemEqual(itemStackIn)) {
                return true;
            }
        }

        for (int j = 0; j < this.mainInventory.length; ++j) {
            if (null != mainInventory[j] && this.mainInventory[j].isItemEqual(itemStackIn)) {
                return true;
            }
        }
