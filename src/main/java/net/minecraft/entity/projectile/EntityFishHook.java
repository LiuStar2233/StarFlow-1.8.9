...
public class EntityFishHook extends Entity {
    ...
    public void onUpdate() {
        ...
        if (0 < fishPosRotationIncrements) {
            ...
            if (!this.worldObj.isRemote) {
                ...
                if (this.angler.isDead || !this.angler.isEntityAlive() || null == itemstack || itemstack.getItem() != Items.fishing_rod || 1024.0D < getDistanceSqToEntity(angler)) {
                    this.setDead();
                    this.angler.fishEntity = null;
                    return;
                }

                if (null != caughtEntity) {
            ...
            for (int i = 0; i < list.size(); ++i) {
                ...
                if (entity1.canBeCollidedWith() && (entity1 != this.angler || 5 <= ticksInAir)) {
                    final float f = 0.3F;
            ...
            if (null != movingobjectposition) {
                if (null != movingobjectposition.entityHit) {
            ...
            if (!this.inGround) {
                ...
                for (this.rotationPitch = (float) (MathHelper.atan2(this.motionY, f5) * 180.0D / Math.PI); -180.0F > rotationPitch - prevRotationPitch; this.prevRotationPitch -= 360.0F) {
                }
                ...
                if (!this.worldObj.isRemote && 0.0D < d10) {
                    WorldServer worldserver = (WorldServer) this.worldObj;
                    int l = 1;
                    BlockPos blockpos = (new BlockPos(this)).up();

                    if (0.25F > rand.nextFloat() && this.worldObj.isRainingAt(blockpos)) {
                        l = 2;
                    }

                    if (0.5F > rand.nextFloat() && !this.worldObj.canSeeSky(blockpos)) {
                        --l;
                    }
                    ...
                    } else if (0 < ticksCatchableDelay) {
                        this.ticksCatchableDelay -= l;

                        if (0 >= ticksCatchableDelay) {
                            this.motionY -= 0.20000000298023224D;
                            this.playSound("random.splash", 0.25F, 1.0F + (this.rand.nextFloat() - this.rand.nextFloat()) * 0.4F);
                            float f8 = (float) MathHelper.floor_double(this.getEntityBoundingBox().minY);
                            worldserver.spawnParticle(EnumParticleTypes.WATER_BUBBLE, this.posX, f8 + 1.0F, this.posZ, (int) (1.0F + this.width * 20.0F), this.width, 0.0D, this.width, 0.20000000298023224D);
                            worldserver.spawnParticle(EnumParticleTypes.WATER_WAKE, this.posX, f8 + 1.0F, this.posZ, (int) (1.0F + this.width * 20.0F), this.width, 0.0D, this.width, 0.20000000298023224D);
    ...
    public void writeEntityToNBT(NBTTagCompound tagCompound) {
        ...
        tagCompound.setString("inTile", null == resourcelocation ? "" : resourcelocation.toString());
    ...
    public void readEntityFromNBT(NBTTagCompound tagCompund) {
        ...
        this.inGround = 1 == tagCompund.getByte("inGround");
    }
