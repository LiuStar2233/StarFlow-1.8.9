    ...
    static void registerDispenserBehaviors() {
        ...
        BlockDispenser.dispenseBehaviorRegistry.putObject(Items.bucket, new BehaviorDefaultDispenseItem() {
            ...
            public ItemStack dispenseStack(IBlockSource source, ItemStack stack) {
                ...
                if (Material.water.equals(material) && block instanceof BlockLiquid && 0 == iblockstate.getValue(BlockLiquid.LEVEL).intValue()) {
                    item = Items.water_bucket;
                } else {
                    if (!Material.lava.equals(material) || !(block instanceof BlockLiquid) || 0 != iblockstate.getValue(BlockLiquid.LEVEL).intValue()) {
                ...
                if (0 == --stack.stackSize) {
                    stack.setItem(item);
                    stack.stackSize = 1;
                } else if (0 > ((TileEntityDispenser) source.getBlockTileEntity()).addItemStack(new ItemStack(item))) {
