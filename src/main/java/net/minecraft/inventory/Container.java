...
public abstract class Container {
    ...
    public void detectAndSendChanges() {
        for (int i = 0; i < this.inventorySlots.size(); ++i) {
            ...
            if (!ItemStack.areItemStacksEqual(itemstack1, itemstack)) {
                itemstack1 = null == itemstack ? null : itemstack.copy();
    ...
    public ItemStack transferStackInSlot(EntityPlayer playerIn, int index) {
        ...
        return null != slot ? slot.getStack() : null;
    }
    ...
    public ItemStack slotClick(int slotId, int clickedButton, int mode, EntityPlayer playerIn) {
        ...
        if (5 == mode) {
            ...
            if ((1 != i || 2 != dragEvent) && i != this.dragEvent) {
                this.resetDrag();
            } else if (null == inventoryplayer.getItemStack()) {
                this.resetDrag();
            } else if (0 == dragEvent) {
                ...
                if (null != slot && canAddItemToSlot(slot, inventoryplayer.getItemStack(), true) && slot.isItemValid(inventoryplayer.getItemStack()) && inventoryplayer.getItemStack().stackSize > this.dragSlots.size() && this.canDragIntoSlot(slot)) {
                ...
                if (!this.dragSlots.isEmpty()) {
                    ...
                    for (Slot slot1 : this.dragSlots) {
                        if (null != slot1 && canAddItemToSlot(slot1, inventoryplayer.getItemStack(), true) && slot1.isItemValid(inventoryplayer.getItemStack()) && inventoryplayer.getItemStack().stackSize >= this.dragSlots.size() && this.canDragIntoSlot(slot1)) {
            ...
            this.resetDrag();
        } else if ((0 == mode || 1 == mode) && (0 == clickedButton || 1 == clickedButton)) {
            if (-999 == slotId) {
                if (null != inventoryplayer.getItemStack()) {
                    if (0 == clickedButton) {
                    ...
                    if (1 == clickedButton) {
                        ...
                        if (0 == inventoryplayer.getItemStack().stackSize) {
                            inventoryplayer.setItemStack(null);
                        }
                    }
                }
                ...
                if (null != slot6 && slot6.canTakeStack(playerIn)) {
                    ...
                    if (null != itemstack8) {
                        ...
                        if (null != slot6.getStack() && slot6.getStack().getItem() == item) {
                ...
                if (null != slot7) {
                    ...
                    if (null == itemstack9) {
                        if (null != itemstack10 && slot7.isItemValid(itemstack10)) {
                            int k2 = 0 == clickedButton ? itemstack10.stackSize : 1;
                            ...
                            if (0 == itemstack10.stackSize) {
                                inventoryplayer.setItemStack(null);
                            }
                        }
                    } else if (slot7.canTakeStack(playerIn)) {
                        if (null == itemstack10) {
                            int j2 = 0 == clickedButton ? itemstack9.stackSize : (itemstack9.stackSize + 1) / 2;
                            ItemStack itemstack12 = slot7.decrStackSize(j2);
                            inventoryplayer.setItemStack(itemstack12);

                            if (0 == itemstack9.stackSize) {
                                slot7.putStack(null);
                            }
                            ...
                            if (itemstack9.getItem() == itemstack10.getItem() && itemstack9.getMetadata() == itemstack10.getMetadata() && ItemStack.areItemStackTagsEqual(itemstack9, itemstack10)) {
                                int i2 = 0 == clickedButton ? itemstack10.stackSize : 1;
                        ...
                        } else if (itemstack9.getItem() == itemstack10.getItem() && 1 < itemstack10.getMaxStackSize() && (!itemstack9.getHasSubtypes() || itemstack9.getMetadata() == itemstack10.getMetadata()) && ItemStack.areItemStackTagsEqual(itemstack9, itemstack10)) {
                            int l1 = itemstack9.stackSize;

                            if (0 < l1 && l1 + itemstack10.stackSize <= itemstack10.getMaxStackSize()) {
        ...
        } else if (2 == mode && 0 <= clickedButton && 9 > clickedButton) {
            ...
            if (slot5.canTakeStack(playerIn)) {
                ...
                boolean flag = null == itemstack7 || slot5.inventory == inventoryplayer && slot5.isItemValid(itemstack7);
                int k1 = -1;
                ...
                if (slot5.getHasStack() && flag) {
                    ...
                    if ((slot5.inventory != inventoryplayer || !slot5.isItemValid(itemstack7)) && null != itemstack7) {
                        if (-1 < k1) {
                ...
                } else if (!slot5.getHasStack() && null != itemstack7 && slot5.isItemValid(itemstack7)) {
                    inventoryplayer.setInventorySlotContents(clickedButton, null);
                    slot5.putStack(itemstack7);
                }
            }
        } else if (3 == mode && playerIn.capabilities.isCreativeMode && null == inventoryplayer.getItemStack() && 0 <= slotId) {
            Slot slot4 = this.inventorySlots.get(slotId);

            if (null != slot4 && slot4.getHasStack()) {
        ...
        } else if (4 == mode && null == inventoryplayer.getItemStack() && 0 <= slotId) {
            Slot slot3 = this.inventorySlots.get(slotId);

            if (null != slot3 && slot3.getHasStack() && slot3.canTakeStack(playerIn)) {
                ItemStack itemstack5 = slot3.decrStackSize(0 == clickedButton ? 1 : slot3.getStack().stackSize);
                slot3.onPickupFromSlot(playerIn, itemstack5);
                playerIn.dropPlayerItemWithRandomChoice(itemstack5, true);
            }
        } else if (6 == mode && 0 <= slotId) {
            Slot slot2 = this.inventorySlots.get(slotId);
            ItemStack itemstack4 = inventoryplayer.getItemStack();

            if (null != itemstack4 && (null == slot2 || !slot2.getHasStack() || !slot2.canTakeStack(playerIn))) {
                int i1 = 0 == clickedButton ? 0 : this.inventorySlots.size() - 1;
                int j1 = 0 == clickedButton ? 1 : -1;

                for (int l2 = 0; 2 > l2; ++l2) {
                    for (int i3 = i1; 0 <= i3 && i3 < this.inventorySlots.size() && itemstack4.stackSize < itemstack4.getMaxStackSize(); i3 += j1) {
                        Slot slot8 = this.inventorySlots.get(i3);

                        if (slot8.getHasStack() && canAddItemToSlot(slot8, itemstack4, true) && slot8.canTakeStack(playerIn) && this.canMergeSlot(itemstack4, slot8) && (0 != l2 || slot8.getStack().stackSize != slot8.getStack().getMaxStackSize())) {
    ...
    protected boolean mergeItemStack(ItemStack stack, int startIndex, int endIndex, boolean reverseDirection) {
        ...
        if (stack.isStackable()) {
            while (0 < stack.stackSize && (!reverseDirection && i < endIndex || reverseDirection && i >= startIndex)) {
                Slot slot = this.inventorySlots.get(i);
                ItemStack itemstack = slot.getStack();

                if (null != itemstack && itemstack.getItem() == stack.getItem() && (!stack.getHasSubtypes() || stack.getMetadata() == itemstack.getMetadata()) && ItemStack.areItemStackTagsEqual(stack, itemstack)) {
    ...
    public static boolean isValidDragMode(int dragModeIn, EntityPlayer player) {
        return 0 == dragModeIn || (1 == dragModeIn || 2 == dragModeIn && player.capabilities.isCreativeMode);
    }
    ...
    public static boolean canAddItemToSlot(Slot slotIn, ItemStack stack, boolean stackSizeMatters) {
        boolean flag = null == slotIn || !slotIn.getHasStack();

        if (null != slotIn && slotIn.getHasStack() && null != stack && stack.isItemEqual(slotIn.getStack()) && ItemStack.areItemStackTagsEqual(slotIn.getStack(), stack)) {
    ...
    public static int calcRedstoneFromInventory(IInventory inv) {
        if (null == inv) {
            ...
            return MathHelper.floor_float(f * 14.0F) + (0 < i ? 1 : 0);
        }
    }
}