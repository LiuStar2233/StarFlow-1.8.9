...
public class ContainerEnchantment extends Container {
    ...
    public ContainerEnchantment(InventoryPlayer playerInv, World worldIn, BlockPos pos) {
        ...
        this.addSlotToContainer(new Slot(this.tableInventory, 1, 35, 47) {
            public boolean isItemValid(ItemStack stack) {
                return stack.getItem() == Items.dye && EnumDyeColor.BLUE == EnumDyeColor.byDyeDamage(stack.getMetadata());
            }
        });
    ...
    public void onCraftMatrixChanged(IInventory inventoryIn) {
        if (inventoryIn == this.tableInventory) {
            ...
            if (null != itemstack && itemstack.isItemEnchantable()) {
                if (!this.worldPointer.isRemote) {
                    int l = 0;

                    for (int j = -1; 1 >= j; ++j) {
                        for (int k = -1; 1 >= k; ++k) {
                            if ((0 != j || 0 != k) && this.worldPointer.isAirBlock(this.position.add(k, 0, j)) && this.worldPointer.isAirBlock(this.position.add(k, 1, j))) {
    ...
    public boolean enchantItem(EntityPlayer playerIn, int id) {
        ...
        if ((null == itemstack1 || itemstack1.stackSize < i) && !playerIn.capabilities.isCreativeMode) {
            return false;
        } else if (0 < enchantLevels[id] && null != itemstack && (playerIn.experienceLevel >= i && playerIn.experienceLevel >= this.enchantLevels[id] || playerIn.capabilities.isCreativeMode)) {
            if (!this.worldPointer.isRemote) {
    ...
    private List<EnchantmentData> func_178148_a(ItemStack stack, int p_178148_2_, int p_178148_3_) {
        ...
        if (stack.getItem() == Items.book && null != list && 1 < list.size()) {
    ...
    public int getLapisAmount() {
        ...
        return null == itemstack ? 0 : itemstack.stackSize;
    }
    ...
    public boolean canInteractWith(EntityPlayer playerIn) {
        return this.worldPointer.getBlockState(this.position).getBlock() == Blocks.enchanting_table && 64.0D >= playerIn.getDistanceSq((double) position.getX() + 0.5D, (double) position.getY() + 0.5D, (double) position.getZ() + 0.5D);
    }
    ...
    public ItemStack transferStackInSlot(EntityPlayer playerIn, int index) {
        ...
        if (null != slot && slot.getHasStack()) {
            ...
            if (0 == index) {
            ...
            } else if (itemstack1.getItem() == Items.dye && EnumDyeColor.BLUE == EnumDyeColor.byDyeDamage(itemstack1.getMetadata())) {
                ...
                if (itemstack1.hasTagCompound() && 1 == itemstack1.stackSize) {
