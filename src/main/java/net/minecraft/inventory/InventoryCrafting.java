...
public class InventoryCrafting implements IInventory {
    ...
    public ItemStack getStackInRowAndColumn(int row, int column) {
        return 0 <= row && row < this.inventoryWidth && 0 <= column && column <= this.inventoryHeight ? this.getStackInSlot(row + column * this.inventoryWidth) : null;
    }
    ...
    public ItemStack decrStackSize(int index, int count) {
        if (null != stackList[index]) {
            if (this.stackList[index].stackSize <= count) {
                ...
                if (0 == stackList[index].stackSize) {
                    this.stackList[index] = null;
                }
