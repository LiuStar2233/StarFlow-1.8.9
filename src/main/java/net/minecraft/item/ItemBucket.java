...
public class ItemBucket extends Item {
        ...
        if (null == movingobjectposition) {
            ...
            if (MovingObjectPosition.MovingObjectType.BLOCK == movingobjectposition.typeOfHit) {
                ...
                if (flag) {
                    ...
                    if (material == Material.water && 0 == iblockstate.getValue(BlockLiquid.LEVEL).intValue()) {
                        worldIn.setBlockToAir(blockpos);
                        playerIn.triggerAchievement(StatList.objectUseStats[Item.getIdFromItem(this)]);
                        return this.fillBucket(itemStackIn, playerIn, Items.water_bucket);
                    }

                    if (material == Material.lava && 0 == iblockstate.getValue(BlockLiquid.LEVEL).intValue()) {
                        worldIn.setBlockToAir(blockpos);
                        playerIn.triggerAchievement(StatList.objectUseStats[Item.getIdFromItem(this)]);
    ...
    private ItemStack fillBucket(ItemStack emptyBuckets, EntityPlayer player, Item fullBucket) {
        if (player.capabilities.isCreativeMode) {
            return emptyBuckets;
        } else if (0 >= --emptyBuckets.stackSize) {
