...
public class ItemSlab extends ItemBlock {
    ...
    public boolean onItemUse(ItemStack stack, EntityPlayer playerIn, World worldIn, BlockPos pos, EnumFacing side, float hitX, float hitY, float hitZ) {
        if (0 == stack.stackSize) {
            ...
            if (iblockstate.getBlock() == this.singleSlab) {
                ...
                if ((EnumFacing.UP == side && BlockSlab.EnumBlockHalf.BOTTOM == blockslab$enumblockhalf || EnumFacing.DOWN == side && BlockSlab.EnumBlockHalf.TOP == blockslab$enumblockhalf) && comparable == object) {
    ...
    public boolean canPlaceBlockOnSide(World worldIn, BlockPos pos, EnumFacing side, EntityPlayer player, ItemStack stack) {
        ...
        if (iblockstate.getBlock() == this.singleSlab) {
            boolean flag = BlockSlab.EnumBlockHalf.TOP == iblockstate.getValue(BlockSlab.HALF);

            if ((EnumFacing.UP == side && !flag || EnumFacing.DOWN == side && flag) && object == iblockstate.getValue(iproperty)) {
                return true;
            }
        }
