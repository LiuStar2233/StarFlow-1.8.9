...
public class NetHandlerPlayServer implements INetHandlerPlayServer, ITickable {
    ...
    public void update() {
        ...
        if (40L < (long) networkTickCount - lastSentPingPacket) {
        ...
        if (0 < chatSpamThresholdCount) {
            --this.chatSpamThresholdCount;
        }

        if (0 < itemDropThreshold) {
            --this.itemDropThreshold;
        }

        if (0L < playerEntity.getLastActiveTime() && 0 < serverController.getMaxPlayerIdleMinutes() && MinecraftServer.getCurrentTimeMillis() - this.playerEntity.getLastActiveTime() > ((long) this.serverController.getMaxPlayerIdleMinutes() * 1000 * 60)) {
        ...
        if (this.func_183006_b(packetIn)) {
            ...
            if (!this.playerEntity.playerConqueredTheEnd) {
                ...
                if (this.hasMoved) {
                    ...
                    if (null != playerEntity.ridingEntity) {
                    ...
                    if (packetIn.isMoving() && -999.0D == packetIn.getPositionY()) {
                    ...
                    if (packetIn.isMoving()) {
                        ...
                        if (3.0E7D < Math.abs(packetIn.getPositionX()) || 3.0E7D < Math.abs(packetIn.getPositionZ())) {
                    ...
                    if (100.0D < d15 - d14 && (!this.serverController.isSinglePlayer() || !this.serverController.getServerOwner().equals(this.playerEntity.getName()))) {
                    ...
                    if (this.playerEntity.onGround && !packetIn.isOnGround() && 0.0D < d12) {
                    ...
                    if (0.0625D < d15 && !this.playerEntity.isPlayerSleeping() && !this.playerEntity.theItemInWorldManager.isCreative()) {
                        flag1 = true;
                ...
                } else if (20 < networkTickCount - field_175090_f) {
                    this.setPlayerLocation(this.lastPosX, this.lastPosY, this.lastPosZ, this.playerEntity.rotationYaw, this.playerEntity.rotationPitch);
                }
            }
        }
        ...
        switch (packetIn.getStatus()) {
            ...
            case STOP_DESTROY_BLOCK:
                ...
                if (36.0D < d3) {
                    ...
                    if (C07PacketPlayerDigging.Action.START_DESTROY_BLOCK == packetIn.getStatus()) {
                        ...
                        if (C07PacketPlayerDigging.Action.STOP_DESTROY_BLOCK == packetIn.getStatus()) {
                            this.playerEntity.theItemInWorldManager.blockRemoving(blockpos);
                        } else if (C07PacketPlayerDigging.Action.ABORT_DESTROY_BLOCK == packetIn.getStatus()) {
    ...
    public void processPlayerBlockPlacement(C08PacketPlayerBlockPlacement packetIn) {
        ...
        if (255 == packetIn.getPlacedBlockDirection()) {
            if (null == itemstack) {
                return;
            }

            this.playerEntity.theItemInWorldManager.tryUseItem(this.playerEntity, worldserver, itemstack);
        } else if (blockpos.getY() < this.serverController.getBuildLimit() - 1 || EnumFacing.UP != enumfacing && blockpos.getY() < this.serverController.getBuildLimit()) {
            if (this.hasMoved && 64.0D > playerEntity.getDistanceSq((double) blockpos.getX() + 0.5D, (double) blockpos.getY() + 0.5D, (double) blockpos.getZ() + 0.5D) && !this.serverController.isBlockProtected(worldserver, blockpos, this.playerEntity) && worldserver.getWorldBorder().contains(blockpos)) {
        ...
        if (null != itemstack && 0 == itemstack.stackSize) {
            this.playerEntity.inventory.mainInventory[this.playerEntity.inventory.currentItem] = null;
            itemstack = null;
        }

        if (null == itemstack || 0 == itemstack.getMaxItemUseDuration()) {
    ...
    public void sendPacket(final Packet packetIn) {
        if (packetIn instanceof S02PacketChat) {
            ...
            if (EntityPlayer.EnumChatVisibility.HIDDEN == entityplayer$enumchatvisibility) {
                return;
            }

            if (EntityPlayer.EnumChatVisibility.SYSTEM == entityplayer$enumchatvisibility && !s02packetchat.isChat()) {
                return;
            }
        }
    ...
    public void processHeldItemChange(C09PacketHeldItemChange packetIn) {
        ...
        if (0 <= packetIn.getSlotId() && packetIn.getSlotId() < InventoryPlayer.getHotbarSize()) {
    ...
    public void processChatMessage(C01PacketChatMessage packetIn) {
        ...
        if (EntityPlayer.EnumChatVisibility.HIDDEN == playerEntity.getChatVisibility()) {
            ...
            if (200 < chatSpamThresholdCount && !this.serverController.getConfigurationManager().canSendCommands(this.playerEntity.getGameProfile())) {
    ...
    public void processUseEntity(C02PacketUseEntity packetIn) {
        ...
        if (null != entity) {
            ...
            if (this.playerEntity.getDistanceSqToEntity(entity) < d0) {
                if (C02PacketUseEntity.Action.INTERACT == packetIn.getAction()) {
                    this.playerEntity.interactWith(entity);
                } else if (C02PacketUseEntity.Action.INTERACT_AT == packetIn.getAction()) {
                    entity.interactAt(this.playerEntity, packetIn.getHitVec());
                } else if (C02PacketUseEntity.Action.ATTACK == packetIn.getAction()) {
        ...
        switch (c16packetclientstatus$enumstate) {
            case PERFORM_RESPAWN:
                if (this.playerEntity.playerConqueredTheEnd) {
                    ...
                    if (0.0F < playerEntity.getHealth()) {
                        return;
                    }
    ...
    public void processCreativeInventoryAction(C10PacketCreativeInventoryAction packetIn) {
        ...
        if (this.playerEntity.theItemInWorldManager.isCreative()) {
            boolean flag = 0 > packetIn.getSlotId();
            ItemStack itemstack = packetIn.getStack();

            if (null != itemstack && itemstack.hasTagCompound() && itemstack.getTagCompound().hasKey("BlockEntityTag", 10)) {
            ...
            boolean flag1 = 1 <= packetIn.getSlotId() && packetIn.getSlotId() < 36 + InventoryPlayer.getHotbarSize();
            boolean flag2 = null == itemstack || null != itemstack.getItem();
            boolean flag3 = null == itemstack || 0 <= itemstack.getMetadata() && 64 >= itemstack.stackSize && 0 < itemstack.stackSize;

            if (flag1 && flag2 && flag3) {
                this.playerEntity.inventoryContainer.putStackInSlot(packetIn.getSlotId(), itemstack);

                this.playerEntity.inventoryContainer.setCanCraft(this.playerEntity, true);
            } else if (flag && flag2 && flag3 && 200 > itemDropThreshold) {
    ...
    public void processConfirmTransaction(C0FPacketConfirmTransaction packetIn) {
        ...
        if (null != oshort && packetIn.getUid() == oshort.shortValue() && this.playerEntity.openContainer.windowId == packetIn.getWindowId() && !this.playerEntity.openContainer.getCanCraft(this.playerEntity) && !this.playerEntity.isSpectator()) {
    ...
    public void processVanilla250Packet(C17PacketCustomPayload packetIn) {
        ...
        if ("MC|BEdit".equals(packetIn.getChannelName())) {
            ...
            if (null != packetIn.getBufferData() && 1 <= packetIn.getBufferData().readableBytes()) {
