...
public abstract class PathNavigate {
    ...
    public boolean tryMoveToEntityLiving(Entity entityIn, double speedIn) {
        ...
        return null != pathentity && this.setPath(pathentity, speedIn);
    }
    ...
    public boolean setPath(PathEntity pathentityIn, double speedIn) {
        if (null == pathentityIn) {
            ...
            if (0 == currentPath.getCurrentPathLength()) {
                return false;
            } else {
                this.speed = speedIn;
    ...
    public void onUpdateNavigation() {
        ...
        if (!this.noPath()) {
            if (this.canNavigate()) {
                this.pathFollow();
            } else if (null != currentPath && this.currentPath.getCurrentPathIndex() < this.currentPath.getCurrentPathLength()) {
                Vec3 vec3 = this.getEntityPosition();
    ...
    protected void checkForStuck(Vec3 positionVec3) {
        if (100 < totalTicks - ticksAtLastPos) {
            if (2.25D > positionVec3.squareDistanceTo(lastPosCheck)) {
    ...
    public boolean noPath() {
        return null == currentPath || this.currentPath.isFinished();
    }
