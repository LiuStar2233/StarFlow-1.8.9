...
public class PathNavigateSwimmer extends PathNavigate {
    ...
    protected void pathFollow() {
        ...
        for (int j = Math.min(this.currentPath.getCurrentPathIndex() + i, this.currentPath.getCurrentPathLength() - 1); j > this.currentPath.getCurrentPathIndex(); --j) {
            ...
            if (36.0D >= vec31.squareDistanceTo(vec3) && this.isDirectPathBetweenPoints(vec3, vec31, 0, 0, 0)) {
    ...
    protected boolean isDirectPathBetweenPoints(Vec3 posVec31, Vec3 posVec32, int sizeX, int sizeY, int sizeZ) {
        ...
        return null == movingobjectposition || MovingObjectPosition.MovingObjectType.MISS == movingobjectposition.typeOfHit;
    }
}