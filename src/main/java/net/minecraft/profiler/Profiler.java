...
public class Profiler {
    ...
    public List<Profiler.Result> getProfilingData(String profilerName) {
        if (!this.profilingEnabled) {
            ...
            for (String s : this.profilingMap.keySet()) {
                if (s.length() > profilerName.length() && s.startsWith(profilerName) && 0 > s.indexOf(".", profilerName.length() + 1)) {
            ...
            for (String s1 : this.profilingMap.keySet()) {
                if (s1.length() > profilerName.length() && s1.startsWith(profilerName) && 0 > s1.indexOf(".", profilerName.length() + 1)) {
    ...
    public String getNameOfLastSection() {
        return 0 == sectionList.size() ? "[UNKNOWN]" : this.sectionList.get(this.sectionList.size() - 1);
    }
