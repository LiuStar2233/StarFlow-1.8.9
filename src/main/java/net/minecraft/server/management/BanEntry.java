...
public abstract class BanEntry<T> extends UserListEntry<T> {
    ...
    public BanEntry(T valueIn, Date startDate, String banner, Date endDate, String banReason) {
        super(valueIn);
        this.banStartDate = null == startDate ? new Date() : startDate;
        this.bannedBy = null == banner ? "(Unknown)" : banner;
        this.banEndDate = endDate;
        this.reason = null == banReason ? "Banned by an operator." : banReason;
    }
    ...
    boolean hasBanExpired() {
        return null != banEndDate && this.banEndDate.before(new Date());
    }

    protected void onSerialization(JsonObject data) {
        data.addProperty("created", dateFormat.format(this.banStartDate));
        data.addProperty("source", this.bannedBy);
        data.addProperty("expires", null == banEndDate ? "forever" : dateFormat.format(this.banEndDate));
