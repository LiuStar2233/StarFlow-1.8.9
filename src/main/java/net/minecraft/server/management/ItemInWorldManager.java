...
public class ItemInWorldManager {
    ...
    public void initializeGameType(WorldSettings.GameType type) {
        if (WorldSettings.GameType.NOT_SET == gameType) {
            this.gameType = type;
        }
    ...
    public void onBlockClicked(BlockPos pos, EnumFacing side) {
        if (this.isCreative()) {
            ...
            if (block.getMaterial() != Material.air && 1.0F <= f) {
                this.tryHarvestBlock(pos);
            } else {
    ...
    public boolean tryHarvestBlock(BlockPos pos) {
        if (this.gameType.isCreative() && null != thisPlayerMP.getHeldItem() && this.thisPlayerMP.getHeldItem().getItem() instanceof ItemSword) {
            return false;
        } else {
            IBlockState iblockstate = this.theWorld.getBlockState(pos);
    ...
    public boolean tryUseItem(EntityPlayer player, World worldIn, ItemStack stack) {
        if (WorldSettings.GameType.SPECTATOR == gameType) {
            ...
            if (itemstack != stack || null != itemstack && (itemstack.stackSize != i || 0 < itemstack.getMaxItemUseDuration() || itemstack.getMetadata() != j)) {
    ...
    public boolean activateBlockOrUseItem(EntityPlayer player, World worldIn, ItemStack stack, BlockPos pos, EnumFacing side, float offsetX, float offsetY, float offsetZ) {
        if (WorldSettings.GameType.SPECTATOR == gameType) {
            ...
            if (!player.isSneaking() || null == player.getHeldItem()) {
