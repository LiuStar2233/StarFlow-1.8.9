...
public class TileEntityBeacon extends TileEntityLockable implements ITickable, IInventory {
     ...
     */
    public void update() {
        if (0L == worldObj.getTotalWorldTime() % 80L) {
            this.updateBeacon();
        }
    }
    ...
    private void addEffectsToPlayers() {
        if (this.isComplete && 0 < levels && !this.worldObj.isRemote && 0 < primaryEffect) {
            double d0 = this.levels * 10 + 10;
            int i = 0;

            if (4 <= levels && this.primaryEffect == this.secondaryEffect) {
                i = 1;
            }
            ...
            if (4 <= levels && this.primaryEffect != this.secondaryEffect && 0 < secondaryEffect) {
    ...
    private void updateSegmentColors() {
        ...
        for (int i1 = k + 1; 256 > i1; ++i1) {
            ...
            if (iblockstate.getBlock() == Blocks.stained_glass) {
                ...
                if (iblockstate.getBlock() != Blocks.stained_glass_pane) {
                    if (15 <= iblockstate.getBlock().getLightOpacity() && iblockstate.getBlock() != Blocks.bedrock) {
        ...
        if (this.isComplete) {
            for (int l1 = 1; 4 >= l1; this.levels = l1++) {
                int i2 = k - l1;
        ...
        if (!this.worldObj.isRemote && 4 == levels && i < this.levels) {
    ...
    private int func_183001_h(int p_183001_1_) {
        if (0 <= p_183001_1_ && p_183001_1_ < Potion.potionTypes.length && null != Potion.potionTypes[p_183001_1_]) {
    ...
    public ItemStack getStackInSlot(int index) {
        return 0 == index ? this.payment : null;
    }
