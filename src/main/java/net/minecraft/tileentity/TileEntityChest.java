...
public class TileEntityChest extends TileEntityLockable implements ITickable, IInventory {
    ...
    public ItemStack decrStackSize(int index, int count) {
        if (null != chestContents[index]) {
            if (this.chestContents[index].stackSize <= count) {
                ...
                if (0 == chestContents[index].stackSize) {
                    this.chestContents[index] = null;
                }
    ...
    public void readFromNBT(NBTTagCompound compound) {
        ...
        for (int i = 0; i < nbttaglist.tagCount(); ++i) {
            ...
            if (0 <= j && j < this.chestContents.length) {
    ...
    public void update() {
        ...
        if (!this.worldObj.isRemote && 0 != numPlayersUsing && 0 == (ticksSinceSync + i + j + k) % 200) {
        ...
        if (0 < numPlayersUsing && 0.0F == lidAngle && null == adjacentChestZNeg && null == adjacentChestXNeg) {
            double d1 = (double) i + 0.5D;
            double d2 = (double) k + 0.5D;

            if (null != adjacentChestZPos) {
                d2 += 0.5D;
            }

            if (null != adjacentChestXPos) {
                d1 += 0.5D;
            }
        ...
        if (0 == numPlayersUsing && 0.0F < lidAngle || 0 < numPlayersUsing && 1.0F > lidAngle) {
            ...
            if (f3 > lidAngle && f3 <= f2 && null == adjacentChestZNeg && null == adjacentChestXNeg) {
    ...
    public int getChestType() {
        if (-1 == cachedChestType) {
            if (null == worldObj || !(this.getBlockType() instanceof BlockChest)) {
                return 0;
            }
