...
public class TileEntityEnchantmentTable extends TileEntity implements ITickable, IInteractionObject {
    ...
    public void update() {
        ...
        if (null != entityplayer) {
            ...
            if (0.5F > bookSpread || 0 == TileEntityEnchantmentTable.rand.nextInt(40)) {
        ...
        while ((float) Math.PI <= bookRotation) {
        ...
        while ((float) Math.PI <= field_145924_q) {
        ...
        for (f2 = this.field_145924_q - this.bookRotation; (float) Math.PI <= f2; f2 -= ((float) Math.PI * 2F)) {
        }

        while (-(float) Math.PI > f2) {
            f2 += ((float) Math.PI * 2F);
        }
