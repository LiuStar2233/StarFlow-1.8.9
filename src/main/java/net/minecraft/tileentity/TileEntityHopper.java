...
public class TileEntityHopper extends TileEntityLockable implements IHopper, ITickable {
    ...
    public void readFromNBT(NBTTagCompound compound) {
        ...
        for (int i = 0; i < nbttaglist.tagCount(); ++i) {
            ...
            if (0 <= j && j < this.inventory.length) {
    ...
    public ItemStack decrStackSize(int index, int count) {
        if (null != inventory[index]) {
            if (this.inventory[index].stackSize <= count) {
                ...
                if (0 == inventory[index].stackSize) {
                    this.inventory[index] = null;
                }
    ...
    private boolean isFull() {
        for (ItemStack itemstack : this.inventory) {
            if (null == itemstack || itemstack.stackSize != itemstack.getMaxStackSize()) {
                return false;
            }
        }
    ...
    private boolean isInventoryFull(IInventory inventoryIn, EnumFacing side) {
        if (inventoryIn instanceof ISidedInventory) {
            ...
            for (int k = 0; k < aint.length; ++k) {
                ...
                if (null == itemstack1 || itemstack1.stackSize != itemstack1.getMaxStackSize()) {
                    return false;
                }
            }
        } else {
    ...
    private static boolean isInventoryEmpty(IInventory inventoryIn, EnumFacing side) {
        if (inventoryIn instanceof ISidedInventory) {
            ...
            for (int i = 0; i < aint.length; ++i) {
                if (null != isidedinventory.getStackInSlot(aint[i])) {
                    return false;
                }
            }
        } else {
            ...
            for (int k = 0; k < j; ++k) {
                if (null != inventoryIn.getStackInSlot(k)) {
                    return false;
                }
            }
        }
    ...
    private static boolean pullItemFromSlot(IHopper hopper, IInventory inventoryIn, int index, EnumFacing direction) {
        ...
        if (null != itemstack && canExtractItemFromSlot(inventoryIn, itemstack, index, direction)) {
            ItemStack itemstack1 = itemstack.copy();
            ItemStack itemstack2 = putStackInInventoryAllSlots(hopper, inventoryIn.decrStackSize(index, 1), null);

            if (null == itemstack2 || 0 == itemstack2.stackSize) {
    ...
    public static ItemStack putStackInInventoryAllSlots(IInventory inventoryIn, ItemStack stack, EnumFacing side) {
        if (inventoryIn instanceof ISidedInventory && null != side) {
            ISidedInventory isidedinventory = (ISidedInventory) inventoryIn;
            int[] aint = isidedinventory.getSlotsForFace(side);

            for (int k = 0; k < aint.length && null != stack && 0 < stack.stackSize; ++k) {
                stack = insertStack(inventoryIn, stack, aint[k], side);
            }
        } else {
            int i = inventoryIn.getSizeInventory();

            for (int j = 0; j < i && null != stack && 0 < stack.stackSize; ++j) {
                stack = insertStack(inventoryIn, stack, j, side);
            }
        }

        if (null != stack && 0 == stack.stackSize) {
            stack = null;
        }
    ...
    public boolean isOnTransferCooldown() {
        return 0 < transferCooldown;
    }

    public boolean mayTransfer() {
        return 1 >= transferCooldown;
    }
