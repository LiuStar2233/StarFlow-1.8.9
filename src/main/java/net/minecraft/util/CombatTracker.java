...
public class CombatTracker {
    ...
    public IChatComponent getDeathMessage() {
        if (0 == combatEntries.size()) {
            ...
            if (null != combatentry && combatentry1.getDamageSrc() == DamageSource.fall) {
                ...
                if (combatentry.getDamageSrc() != DamageSource.fall && combatentry.getDamageSrc() != DamageSource.outOfWorld) {
                    if (null != ichatcomponent2 && (!ichatcomponent2.equals(ichatcomponent1))) {
                        ...
                        if (null != itemstack1 && itemstack1.hasDisplayName()) {
    ...
    public EntityLivingBase func_94550_c() {
        ...
        for (CombatEntry combatentry : this.combatEntries) {
            if (combatentry.getDamageSrc().getEntity() instanceof EntityPlayer && (null == entityplayer || combatentry.func_94563_c() > f1)) {
                f1 = combatentry.func_94563_c();
                entityplayer = (EntityPlayer) combatentry.getDamageSrc().getEntity();
            }

            if (combatentry.getDamageSrc().getEntity() instanceof EntityLivingBase && (null == entitylivingbase || combatentry.func_94563_c() > f)) {
                f = combatentry.func_94563_c();
                entitylivingbase = (EntityLivingBase) combatentry.getDamageSrc().getEntity();
            }
        }

        if (null != entityplayer && f1 >= f / 3.0F) {
            return entityplayer;
        } else {
    ...
    private CombatEntry func_94544_f() {
        ...
        for (int j = 0; j < this.combatEntries.size(); ++j) {
            ...
            CombatEntry combatentry3 = 0 < j ? this.combatEntries.get(j - 1) : null;

            if ((combatentry2.getDamageSrc() == DamageSource.fall || combatentry2.getDamageSrc() == DamageSource.outOfWorld) && 0.0F < combatentry2.getDamageAmount() && (null == combatentry || combatentry2.getDamageAmount() > f)) {
                if (0 < j) {
            ...
            if (null != combatentry2.func_94562_g() && (null == combatentry1 || (float) i < combatentry2.func_94563_c())) {
                combatentry1 = combatentry2;
            }
        }

        if (5.0F < f && null != combatentry) {
            return combatentry;
        } else if (5 < i && null != combatentry1) {
            return combatentry1;
        } else {
            return null;
        }
    ...
    private String func_94548_b(CombatEntry p_94548_1_) {
        return null == p_94548_1_.func_94562_g() ? "generic" : p_94548_1_.func_94562_g();
    }
