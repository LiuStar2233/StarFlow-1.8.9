...
public class DamageSource {
    ...
    public static DamageSource causeFireballDamage(EntityFireball fireball, Entity indirectEntityIn) {
        return null == indirectEntityIn ? (new EntityDamageSourceIndirect("onFire", fireball, fireball)).setFireDamage().setProjectile() : (new EntityDamageSourceIndirect("fireball", fireball, indirectEntityIn)).setFireDamage().setProjectile();
    }
    ...
    public static DamageSource setExplosionSource(Explosion explosionIn) {
        return null != explosionIn && null != explosionIn.getExplosivePlacedBy() ? (new EntityDamageSource("explosion.player", explosionIn.getExplosivePlacedBy())).setDifficultyScaled().setExplosion() : (new DamageSource("explosion")).setDifficultyScaled().setExplosion();
    }
    ...
    public IChatComponent getDeathMessage(EntityLivingBase entityLivingBaseIn) {
        ...
        String s1 = s + ".player";
        return null != entitylivingbase && StatCollector.canTranslate(s1) ? new ChatComponentTranslation(s1, entityLivingBaseIn.getDisplayName(), entitylivingbase.getDisplayName()) : new ChatComponentTranslation(s, entityLivingBaseIn.getDisplayName());
    }
