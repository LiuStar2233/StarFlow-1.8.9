...
public class ChunkCache implements IBlockAccess {
    ...
    public ChunkCache(World worldIn, BlockPos posFromIn, BlockPos posToIn, int subIn) {
        ...
        for (int i1 = posFromIn.getX() >> 4; i1 <= posToIn.getX() >> 4; ++i1) {
            for (int j1 = posFromIn.getZ() >> 4; j1 <= posToIn.getZ() >> 4; ++j1) {
                ...
                if (null != chunk && !chunk.getAreLevelsEmpty(posFromIn.getY(), posToIn.getY())) {
    ...
    public IBlockState getBlockState(BlockPos pos) {
        if (0 <= pos.getY() && 256 > pos.getY()) {
            ...
            if (0 <= i && i < this.chunkArray.length && 0 <= j && j < this.chunkArray[i].length) {
    ...
    private int getLightForExt(EnumSkyBlock p_175629_1_, BlockPos pos) {
        if (EnumSkyBlock.SKY == p_175629_1_ && this.worldObj.provider.getHasNoSky()) {
            return 0;
        } else if (0 <= pos.getY() && 256 > pos.getY()) {
