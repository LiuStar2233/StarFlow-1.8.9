...
public class DifficultyInstance {
    ...
    public float getClampedAdditionalDifficulty() {
        return 2.0F > additionalDifficulty ? 0.0F : (4.0F < additionalDifficulty ? 1.0F : (this.additionalDifficulty - 2.0F) / 2.0F);
    }
    ...
    private float calculateAdditionalDifficulty(EnumDifficulty difficulty, long worldTime, long chunkInhabitedTime, float moonPhaseFactor) {
        if (EnumDifficulty.PEACEFUL == difficulty) {
            return 0.0F;
        } else {
            boolean flag = EnumDifficulty.HARD == difficulty;
            float f = 0.75F;
            ...
            if (EnumDifficulty.EASY == difficulty) {
                f2 *= 0.5F;
            }
