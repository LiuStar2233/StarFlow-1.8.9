...
public class Explosion {
            ...
            for (int k = 0; 16 > k; ++k) {
                for (int l = 0; 16 > l; ++l) {
                    if (0 == j || 15 == j || 0 == k || 15 == k || 0 == l || 15 == l) {
                        ...
                        for (final float f1 = 0.3F; 0.0F < f; f -= 0.22500001F) {
                            BlockPos blockpos = new BlockPos(d4, d6, d8);
                            IBlockState iblockstate = this.worldObj.getBlockState(blockpos);

                            if (iblockstate.getBlock().getMaterial() != Material.air) {
                                float f2 = null != exploder ? this.exploder.getExplosionResistance(this, this.worldObj, blockpos, iblockstate) : iblockstate.getBlock().getExplosionResistance(null);
                                f -= (f2 + 0.3F) * 0.3F;
                            }

                            if (0.0F < f && (null == exploder || this.exploder.verifyExplosion(this, this.worldObj, blockpos, iblockstate, f))) {
                                set.add(blockpos);
                            }
    ...
    public void doExplosionB(boolean spawnParticles) {
        ...
        if (2.0F <= explosionSize && this.isSmoking) {
        ...
        if (this.isFlaming) {
            for (BlockPos blockpos1 : this.affectedBlockPositions) {
                if (this.worldObj.getBlockState(blockpos1).getBlock().getMaterial() == Material.air && this.worldObj.getBlockState(blockpos1.down()).getBlock().isFullBlock() && 0 == explosionRNG.nextInt(3)) {
    ...
    public EntityLivingBase getExplosivePlacedBy() {
        return null == exploder ? null : (this.exploder instanceof EntityTNTPrimed ? ((EntityTNTPrimed) this.exploder).getTntPlacedBy() : (this.exploder instanceof EntityLivingBase ? (EntityLivingBase) this.exploder : null));
    }
