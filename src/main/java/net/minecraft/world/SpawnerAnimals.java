...
public final class SpawnerAnimals {
        ...
        if (!spawnHostileMobs && !spawnPeacefulMobs) {
            ...
            for (EntityPlayer entityplayer : worldServerIn.playerEntities) {
                if (!entityplayer.isSpectator()) {
                    ...
                    for (int i1 = -l; l >= i1; ++i1) {
                        for (int j1 = -l; l >= j1; ++j1) {
                            boolean flag = -l == i1 || l == i1 || -l == j1 || l == j1;
                            ...
                            if (!block.isNormalCube()) {
                                ...
                                for (int k2 = 0; 3 > k2; ++k2) {
                                    ...
                                    for (int l3 = 0; 4 > l3; ++l3) {
                                        ...
                                        if (!worldServerIn.isAnyPlayerWithinRangeAt(f, i3, f1, 24.0D) && 576.0D <= blockpos2.distanceSq(f, i3, f1)) {
                                            if (null == biomegenbase$spawnlistentry) {
    ...
    private static BlockPos getRandomChunkPosition(World worldIn, int x, int z) {
        ...
        int l = worldIn.rand.nextInt(0 < k ? k : chunk.getTopFilledSegment() + 16 - 1);
    ...
    public static boolean canCreatureTypeSpawnAtLocation(EntityLiving.SpawnPlacementType spawnPlacementTypeIn, World worldIn, BlockPos pos) {
        if (!worldIn.getWorldBorder().contains(pos)) {
            ...
            if (EntityLiving.SpawnPlacementType.IN_WATER == spawnPlacementTypeIn) {
