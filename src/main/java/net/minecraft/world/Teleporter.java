...
public class Teleporter {
    ...
    public void placeInPortal(Entity entityIn, float rotationYaw) {
        if (1 != worldServerInstance.provider.getDimensionId()) {
            ...
            for (int j1 = -2; 2 >= j1; ++j1) {
                for (int k1 = -2; 2 >= k1; ++k1) {
                    for (int l1 = -1; 3 > l1; ++l1) {
                        int i2 = i + k1 * l + j1 * i1;
                        int j2 = j + l1;
                        int k2 = k + k1 * i1 - j1 * l;
                        boolean flag = 0 > l1;
    ...
    public boolean placeInExistingPortal(Entity entityIn, float rotationYaw) {
        ...
        if (this.destinationCoordinateCache.containsItem(l)) {
            ...
            for (int i1 = -128; 128 >= i1; ++i1) {
                BlockPos blockpos2;

                for (int j1 = -128; 128 >= j1; ++j1) {
                    for (BlockPos blockpos1 = blockpos3.add(i1, this.worldServerInstance.getActualHeight() - 1 - blockpos3.getY(), j1); 0 <= blockpos1.getY(); blockpos1 = blockpos2) {
        ...
        if (0.0D <= d0) {
            ...
            boolean flag1 = EnumFacing.AxisDirection.NEGATIVE == blockpattern$patternhelper.getFinger().rotateY().getAxisDirection();
            double d2 = EnumFacing.Axis.X == blockpattern$patternhelper.getFinger().getAxis() ? (double) blockpattern$patternhelper.getPos().getZ() : (double) blockpattern$patternhelper.getPos().getX();
            ...
            if (EnumFacing.Axis.X == blockpattern$patternhelper.getFinger().getAxis()) {
    ...
    public boolean makePortal(Entity entityIn) {
        ...
        for (int j2 = j - i; j2 <= j + i; ++j2) {
            ...
            for (int l2 = l - i; l2 <= l + i; ++l2) {
                ...
                for (int j3 = this.worldServerInstance.getActualHeight() - 1; 0 <= j3; --j3) {
                    if (this.worldServerInstance.isAirBlock(blockpos$mutableblockpos.set(j2, j3, l2))) {
                        while (0 < j3 && this.worldServerInstance.isAirBlock(blockpos$mutableblockpos.set(j2, j3 - 1, l2))) {
                            --j3;
                        }
                        ...
                        for (int k3 = i2; k3 < i2 + 4; ++k3) {
                            ...
                            for (int j4 = 0; 3 > j4; ++j4) {
                                for (int k4 = 0; 4 > k4; ++k4) {
                                    for (int l4 = -1; 4 > l4; ++l4) {
                                        int i5 = j2 + (k4 - 1) * l3 + j4 * i4;
                                        int j5 = j3 + l4;
                                        int k5 = l2 + (k4 - 1) * i4 - j4 * l3;
                                        blockpos$mutableblockpos.set(i5, j5, k5);

                                        if (0 > l4 && !this.worldServerInstance.getBlockState(blockpos$mutableblockpos).getBlock().getMaterial().isSolid() || 0 <= l4 && !this.worldServerInstance.isAirBlock(blockpos$mutableblockpos)) {
                                            continue label142;
                                        }
                                    }
                                }
        ...
        if (0.0D > d0) {
            for (int l5 = j - i; l5 <= j + i; ++l5) {
                ...
                for (int j6 = l - i; j6 <= l + i; ++j6) {
                    ...
                    for (int i7 = this.worldServerInstance.getActualHeight() - 1; 0 <= i7; --i7) {
                        if (this.worldServerInstance.isAirBlock(blockpos$mutableblockpos.set(l5, i7, j6))) {
                            while (0 < i7 && this.worldServerInstance.isAirBlock(blockpos$mutableblockpos.set(l5, i7 - 1, j6))) {
                                --i7;
                            }
                            ...
                            for (int k7 = i2; k7 < i2 + 2; ++k7) {
                                ...
                                for (int j10 = 0; 4 > j10; ++j10) {
                                    for (int j11 = -1; 4 > j11; ++j11) {
                                        int j12 = l5 + (j10 - 1) * j8;
                                        int i13 = i7 + j11;
                                        int j13 = j6 + (j10 - 1) * j9;
                                        blockpos$mutableblockpos.set(j12, i13, j13);

                                        if (0 > j11 && !this.worldServerInstance.getBlockState(blockpos$mutableblockpos).getBlock().getMaterial().isSolid() || 0 <= j11 && !this.worldServerInstance.isAirBlock(blockpos$mutableblockpos)) {
                                            continue label562;
                                        }
                                    }
                                }
        ...
        if (0.0D > d0) {
            ...
            for (int j7 = -1; 1 >= j7; ++j7) {
                for (int l7 = 1; 3 > l7; ++l7) {
                    for (int k8 = -1; 3 > k8; ++k8) {
        ...
        IBlockState iblockstate = Blocks.portal.getDefaultState().withProperty(BlockPortal.AXIS, 0 != l6 ? EnumFacing.Axis.X : EnumFacing.Axis.Z);

        for (int i8 = 0; 4 > i8; ++i8) {
            for (int l8 = 0; 4 > l8; ++l8) {
                for (int l9 = -1; 4 > l9; ++l9) {
                    int l10 = i6 + (l8 - 1) * l6;
                    int l11 = k2 + l9;
                    int k12 = k6 + (l8 - 1) * i3;
                    boolean flag1 = 0 == l8 || 3 == l8 || -1 == l9 || 3 == l9;
    ...
    public void removeStalePortalLocations(long worldTime) {
        if (0L == worldTime % 100L) {
            ...
            while (iterator.hasNext()) {
                ...
                if (null == teleporter$portalposition || teleporter$portalposition.lastUpdateTime < i) {
                    iterator.remove();
