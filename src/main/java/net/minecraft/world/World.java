...
public abstract class World implements IBlockAccess {
    ...
    private boolean isValid(BlockPos pos) {
        return -30000000 <= pos.getX() && -30000000 <= pos.getZ() && 30000000 > pos.getX() && 30000000 > pos.getZ() && 0 <= pos.getY() && 256 > pos.getY();
    }
    ...
    public boolean setBlockState(BlockPos pos, IBlockState newState, int flags) {
        if (!this.isValid(pos)) {
            ...
            if (null == iblockstate) {
                ...
                if (0 != (flags & 2) && (!this.isRemote || 0 == (flags & 4)) && chunk.isPopulated()) {
                    this.markBlockForUpdate(pos);
                }

                if (!this.isRemote && 0 != (flags & 1)) {
    ...
    public void notifyNeighborsOfStateExcept(BlockPos pos, Block blockType, EnumFacing skipSide) {
        if (EnumFacing.WEST != skipSide) {
            this.notifyBlockOfStateChange(pos.west(), blockType);
        }

        if (EnumFacing.EAST != skipSide) {
            this.notifyBlockOfStateChange(pos.east(), blockType);
        }

        if (EnumFacing.DOWN != skipSide) {
            this.notifyBlockOfStateChange(pos.down(), blockType);
        }

        if (EnumFacing.UP != skipSide) {
            this.notifyBlockOfStateChange(pos.up(), blockType);
        }

        if (EnumFacing.NORTH != skipSide) {
            this.notifyBlockOfStateChange(pos.north(), blockType);
        }

        if (EnumFacing.SOUTH != skipSide) {
            this.notifyBlockOfStateChange(pos.south(), blockType);
        }
    }
        ...
        if (pos.getY() >= this.getSeaLevel()) {
            ...
            if (!this.canSeeSky(blockpos)) {
                ...
                for (blockpos = blockpos.down(); blockpos.getY() > pos.getY(); blockpos = blockpos.down()) {
                    ...
                    if (0 < block.getLightOpacity() && !block.getMaterial().isLiquid()) {
                        return false;
                    }
                }
    ...
    public int getLight(BlockPos pos, boolean checkNeighbors) {
        if (-30000000 <= pos.getX() && -30000000 <= pos.getZ() && 30000000 > pos.getX() && 30000000 > pos.getZ()) {
    ...
    public int getChunksLowestHorizon(int x, int z) {
        if (-30000000 <= x && -30000000 <= z && 30000000 > x && 30000000 > z) {
    ...
    public int getLightFromNeighborsFor(EnumSkyBlock type, BlockPos pos) {
        if (this.provider.getHasNoSky() && EnumSkyBlock.SKY == type) {
            return 0;
        } else {
            if (0 > pos.getY()) {
    ...
    public MovingObjectPosition rayTraceBlocks(Vec3 vec31, Vec3 vec32, boolean stopOnLiquid, boolean ignoreBlockWithoutBoundingBox, boolean returnLastUncollidableBlock) {
        if (!Double.isNaN(vec31.xCoord) && !Double.isNaN(vec31.yCoord) && !Double.isNaN(vec31.zCoord)) {
            if (!Double.isNaN(vec32.xCoord) && !Double.isNaN(vec32.yCoord) && !Double.isNaN(vec32.zCoord)) {
                ...
                if ((!ignoreBlockWithoutBoundingBox || null != block.getCollisionBoundingBox(this, blockpos, iblockstate)) && block.canCollideCheck(iblockstate, stopOnLiquid)) {
                    MovingObjectPosition movingobjectposition = block.collisionRayTrace(this, blockpos, vec31, vec32);

                    if (null != movingobjectposition) {
                ...
                while (0 <= k1--) {
                    ...
                    l = MathHelper.floor_double(vec31.xCoord) - (EnumFacing.EAST == enumfacing ? 1 : 0);
                    i1 = MathHelper.floor_double(vec31.yCoord) - (EnumFacing.UP == enumfacing ? 1 : 0);
                    j1 = MathHelper.floor_double(vec31.zCoord) - (EnumFacing.SOUTH == enumfacing ? 1 : 0);
                    blockpos = new BlockPos(l, i1, j1);
                    IBlockState iblockstate1 = this.getBlockState(blockpos);
                    Block block1 = iblockstate1.getBlock();

                    if (!ignoreBlockWithoutBoundingBox || null != block1.getCollisionBoundingBox(this, blockpos, iblockstate1)) {
                        if (block1.canCollideCheck(iblockstate1, stopOnLiquid)) {
                            MovingObjectPosition movingobjectposition1 = block1.collisionRayTrace(this, blockpos, vec31, vec32);

                            if (null != movingobjectposition1) {
    ...
    public void removeEntity(Entity entityIn) {
        if (null != entityIn.riddenByEntity) {
            entityIn.riddenByEntity.mountEntity(null);
        }

        if (null != entityIn.ridingEntity) {
    ...
    public List<AxisAlignedBB> getCollidingBoundingBoxes(Entity entityIn, AxisAlignedBB bb) {
        ...
        for (int j2 = 0; j2 < list1.size(); ++j2) {
            if (entityIn.riddenByEntity != list1 && entityIn.ridingEntity != list1) {
                ...
                if (null != axisalignedbb && axisalignedbb.intersectsWith(bb)) {
            ...
            for (int l1 = i1; l1 < j1; ++l1) {
                if (this.isBlockLoaded(blockpos$mutableblockpos.set(k1, 64, l1))) {
                    for (int i2 = k - 1; i2 < l; ++i2) {
                        ...
                        if (-30000000 <= k1 && 30000000 > k1 && -30000000 <= l1 && 30000000 > l1) {
    ...
    public BlockPos getTopSolidOrLiquidBlock(BlockPos pos) {
        ...
        for (blockpos = new BlockPos(pos.getX(), chunk.getTopFilledSegment() + 16, pos.getZ()); 0 <= blockpos.getY(); blockpos = blockpos1) {
    ...
    public void updateEntityWithOptionalForce(Entity entityIn, boolean forceUpdate) {
        ...
        if (!forceUpdate || this.isAreaLoaded(i - k, 0, j - k, i + k, 0, j + k, true)) {
            ...
            if (forceUpdate && entityIn.addedToChunk && null != entityIn.riddenByEntity) {
    ...
    public boolean checkNoEntityCollision(AxisAlignedBB bb, Entity entityIn) {
        ...
        for (int i = 0; i < list.size(); ++i) {
            ...
            if (!entity.isDead && entity.preventEntitySpawning && entity != entityIn && (null == entityIn || entityIn.ridingEntity != entity && entityIn.riddenByEntity != entity)) {
                return false;
            }
        }
    ...
    public boolean handleMaterialAcceleration(AxisAlignedBB bb, Material materialIn, Entity entityIn) {
        ...
        if (!this.isAreaLoaded(i, k, i1, j, l, j1, true)) {
            ...
            if (0.0D < vec3.lengthVector() && entityIn.isPushedByWater()) {
                vec3 = vec3.normalize();
    ...
    public float getBlockDensity(Vec3 vec, AxisAlignedBB bb) {
        ...
        if (0.0D <= d0 && 0.0D <= d1 && 0.0D <= d2) {
            int i = 0;
            int j = 0;

            for (float f = 0.0F; 1.0F >= f; f = (float) ((double) f + d0)) {
                for (float f1 = 0.0F; 1.0F >= f1; f1 = (float) ((double) f1 + d1)) {
                    for (float f2 = 0.0F; 1.0F >= f2; f2 = (float) ((double) f2 + d2)) {
                        double d5 = bb.minX + (bb.maxX - bb.minX) * (double) f;
                        double d6 = bb.minY + (bb.maxY - bb.minY) * (double) f1;
                        double d7 = bb.minZ + (bb.maxZ - bb.minZ) * (double) f2;

                        if (null == rayTraceBlocks(new Vec3(d5 + d3, d6, d7 + d4), vec)) {
                            ++i;
                        }
    ...
    public void setTileEntity(BlockPos pos, TileEntity tileEntityIn) {
        if (null != tileEntityIn && !tileEntityIn.isInvalid()) {
            if (this.processingLoadedTiles) {
    ...
    public void removeTileEntity(BlockPos pos) {
        ...
        if (null != tileentity && this.processingLoadedTiles) {
            tileentity.invalidate();
    ...
    public boolean isBlockFullCube(BlockPos pos) {
        ...
        return null != axisalignedbb && 1.0D <= axisalignedbb.getAverageEdgeLength();
    }
    ...
    public static boolean doesBlockHaveSolidTopSurface(IBlockAccess blockAccess, BlockPos pos) {
        ...
        Block block = iblockstate.getBlock();
        return block.getMaterial().isOpaque() && block.isFullCube() || (block instanceof BlockStairs ? BlockStairs.EnumHalf.TOP == iblockstate.getValue(BlockStairs.HALF) : (block instanceof BlockSlab ? BlockSlab.EnumBlockHalf.TOP == iblockstate.getValue(BlockSlab.HALF) : (block instanceof BlockHopper || (block instanceof BlockSnow && 7 == iblockstate.getValue(BlockSnow.LAYERS).intValue()))));
    }
    ...
    protected void playMoodSoundAndCheckLight(int p_147467_1_, int p_147467_2_, Chunk chunkIn) {
        ...
        if (0 == ambientTickCountdown && !this.isRemote) {
            ...
            if (block.getMaterial() == Material.air && this.getLight(blockpos) <= this.rand.nextInt(8) && 0 >= getLightFor(EnumSkyBlock.SKY, blockpos)) {
                EntityPlayer entityplayer = this.getClosestPlayer((double) j + 0.5D, (double) l + 0.5D, (double) k + 0.5D, 8.0D);

                if (null != entityplayer && 4.0D < entityplayer.getDistanceSq((double) j + 0.5D, (double) l + 0.5D, (double) k + 0.5D)) {
    ...
    public boolean canBlockFreeze(BlockPos pos, boolean noWaterAdj) {
        ...
        if (0.15F < f) {
            ...
            if (0 <= pos.getY() && 256 > pos.getY() && 10 > getLightFor(EnumSkyBlock.BLOCK, pos)) {
                IBlockState iblockstate = this.getBlockState(pos);
                Block block = iblockstate.getBlock();

                if ((block == Blocks.water || block == Blocks.flowing_water) && 0 == iblockstate.getValue(BlockLiquid.LEVEL).intValue()) {
                    if (!noWaterAdj) {
                        return true;
                    }
    ...
    private int getRawLight(BlockPos pos, EnumSkyBlock lightType) {
        if (EnumSkyBlock.SKY == lightType && this.canSeeSky(pos)) {
            return 15;
        } else {
            Block block = this.getBlockState(pos).getBlock();
            int i = EnumSkyBlock.SKY == lightType ? 0 : block.getLightValue();
            int j = block.getLightOpacity();

            if (15 <= j && 0 < block.getLightValue()) {
                j = 1;
            }
    ...
    public boolean canBlockBePlaced(Block blockIn, BlockPos pos, boolean p_175716_3_, EnumFacing side, Entity entityIn, ItemStack itemStackIn) {
        ...
        return (null == axisalignedbb || this.checkNoEntityCollision(axisalignedbb, entityIn)) && (block.getMaterial() == Material.circuits && blockIn == Blocks.anvil || block.getMaterial().isReplaceable() && blockIn.canReplace(this, pos, side, itemStackIn));
    }
    ...
    public boolean isSidePowered(BlockPos pos, EnumFacing side) {
        return 0 < getRedstonePower(pos, side);
    }
    ...
    public boolean isBlockPowered(BlockPos pos) {
        return 0 < getRedstonePower(pos.down(), EnumFacing.DOWN) || (0 < getRedstonePower(pos.up(), EnumFacing.UP) || (0 < getRedstonePower(pos.north(), EnumFacing.NORTH) || (0 < getRedstonePower(pos.south(), EnumFacing.SOUTH) || (0 < getRedstonePower(pos.west(), EnumFacing.WEST) || 0 < getRedstonePower(pos.east(), EnumFacing.EAST)))));
    }
    ...
    public EntityPlayer getClosestPlayer(double x, double y, double z, double distance) {
        ...
        for (int i = 0; i < this.playerEntities.size(); ++i) {
            ...
            if (EntitySelectors.NOT_SPECTATING.apply(entityplayer1)) {
                ...
                if ((0.0D > distance || d1 < distance * distance) && (-1.0D == d0 || d1 < d0)) {
                    d0 = d1;
    ...
    public boolean isThundering() {
        return 0.9D < (double) getThunderStrength(1.0F);
    }
    ...
    public boolean isRaining() {
        return 0.2D < (double) getRainStrength(1.0F);
    }
    ...
    public CrashReportCategory addWorldInfoToCrashReport(CrashReport report) {
        ...
        crashreportcategory.addCrashSection("Level name", null == worldInfo ? "????" : this.worldInfo.getWorldName());
