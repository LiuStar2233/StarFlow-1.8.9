...
public abstract class BiomeGenBase {
    ...
    protected BiomeGenBase setTemperatureRainfall(float temperatureIn, float rainfallIn) {
        if (0.1F < temperatureIn && 0.2F > temperatureIn) {
    ...
    public WorldGenAbstractTree genBigTreeChance(Random rand) {
        return 0 == rand.nextInt(10) ? this.worldGeneratorBigTree : this.worldGeneratorTrees;
    }
    ...
    public BlockFlower.EnumFlowerType pickRandomFlower(Random rand, BlockPos pos) {
        return 0 < rand.nextInt(3) ? BlockFlower.EnumFlowerType.DANDELION : BlockFlower.EnumFlowerType.POPPY;
    }
            ...
            if (j1 <= rand.nextInt(5)) {
                ...
                if (iblockstate2.getBlock().getMaterial() == Material.air) {
                    ...
                    if (-1 == j) {
                        ...
                        if (j1 < i && (null == iblockstate || iblockstate.getBlock().getMaterial() == Material.air)) {
                            if (0.15F > getFloatTemperature(blockpos$mutableblockpos.set(x, j1, z))) {
                        ...
                        if (0 == j && iblockstate1.getBlock() == Blocks.sand) {
                            j = rand.nextInt(4) + Math.max(0, j1 - 63);
                            iblockstate1 = BlockSand.EnumType.RED_SAND == iblockstate1.getValue(BlockSand.VARIANT) ? Blocks.red_sandstone.getDefaultState() : Blocks.sandstone.getDefaultState();
                        }
                    }
                }
            }
    ...
    public boolean isEqualTo(BiomeGenBase biome) {
        return biome == this || (null != biome && this.getBiomeClass() == biome.getBiomeClass());
    }

    public BiomeGenBase.TempCategory getTempCategory() {
        return 0.2D > (double) temperature ? BiomeGenBase.TempCategory.COLD : (1.0D > (double) temperature ? BiomeGenBase.TempCategory.MEDIUM : BiomeGenBase.TempCategory.WARM);
    }
    ...
    public static BiomeGenBase getBiomeFromBiomeList(int biomeId, BiomeGenBase biome) {
        if (0 <= biomeId && biomeId <= biomeList.length) {
            BiomeGenBase biomegenbase = biomeList[biomeId];
            return null == biomegenbase ? biome : biomegenbase;
        } else {
