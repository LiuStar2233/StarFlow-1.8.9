...
public class Chunk {
    ...
    public int getTopFilledSegment() {
        for (int i = this.storageArrays.length - 1; 0 <= i; --i) {
            if (null != storageArrays[i]) {
    ...
    protected void generateHeightMap() {
        ...
        for (int j = 0; 16 > j; ++j) {
            for (int k = 0; 16 > k; ++k) {
                ...
                for (int l = i + 16; 0 < l; --l) {
                    Block block = this.getBlock0(j, l - 1, k);

                    if (0 != block.getLightOpacity()) {
    ...
    public void generateSkylightMap() {
        ...
        for (int j = 0; 16 > j; ++j) {
            for (int k = 0; 16 > k; ++k) {
                ...
                for (int l = i + 16; 0 < l; --l) {
                    if (0 != getBlockLightOpacity(j, l - 1, k)) {
                        this.heightMap[k << 4 | j] = l;
    ...
    private void relightBlock(int x, int y, int z) {
        ...
        while (0 < j && 0 == getBlockLightOpacity(x, j - 1, z)) {
            --j;
        }
    ...
    private Block getBlock0(int x, int y, int z) {
        ...
        if (0 <= y && y >> 4 < this.storageArrays.length) {
    ...
    public IBlockState getBlockState(final BlockPos pos) {
        if (this.worldObj.getWorldType() == WorldType.DEBUG_WORLD) {
            ...
            return null == iblockstate ? Blocks.air.getDefaultState() : iblockstate;
        } else {
            try {
                if (0 <= pos.getY() && pos.getY() >> 4 < this.storageArrays.length) {
    ...
    private int getBlockMetadata(int x, int y, int z) {
        if (y >> 4 >= this.storageArrays.length) {
            ...
            ExtendedBlockStorage extendedblockstorage = this.storageArrays[y >> 4];
            return null != extendedblockstorage ? extendedblockstorage.getExtBlockMetadata(x, y & 15, z) : 0;
        }
    }
        ...
        if (iblockstate == state) {
            ...
            if (extendedblockstorage.getBlockByExtId(i, j & 15, k) != block) {
                ...
                if (flag) {
                    ...
                    if (j1 != k1 && (j1 < k1 || 0 < getLightFor(EnumSkyBlock.SKY, pos) || 0 < getLightFor(EnumSkyBlock.BLOCK, pos))) {
    ...
    public int getLightFor(EnumSkyBlock p_177413_1_, BlockPos pos) {
        ...
        int k = pos.getZ() & 15;
        ExtendedBlockStorage extendedblockstorage = this.storageArrays[j >> 4];
        return null == extendedblockstorage ? (this.canSeeSky(pos) ? p_177413_1_.defaultLightValue : 0) : (EnumSkyBlock.SKY == p_177413_1_ ? (this.worldObj.provider.getHasNoSky() ? 0 : extendedblockstorage.getExtSkylightValue(i, j & 15, k)) : (EnumSkyBlock.BLOCK == p_177413_1_ ? extendedblockstorage.getExtBlocklightValue(i, j & 15, k) : p_177413_1_.defaultLightValue));
    }
    ...
    public void setLightFor(EnumSkyBlock p_177431_1_, BlockPos pos, int value) {
        ...
        if (EnumSkyBlock.SKY == p_177431_1_) {
            if (!this.worldObj.provider.getHasNoSky()) {
                extendedblockstorage.setExtSkylightValue(i, j & 15, k, value);
            }
        } else if (EnumSkyBlock.BLOCK == p_177431_1_) {
    ...
    public TileEntity getTileEntity(BlockPos pos, Chunk.EnumCreateEntityType p_177424_2_) {
        ...
        if (null == tileentity) {
            if (EnumCreateEntityType.IMMEDIATE == p_177424_2_) {
                tileentity = this.createNewTileEntity(pos);
                this.worldObj.setTileEntity(pos, tileentity);
            } else if (EnumCreateEntityType.QUEUED == p_177424_2_) {
            ...
            if (!this.entityLists[k].isEmpty()) {
                for (Entity entity : this.entityLists[k]) {
                    if (entity.getEntityBoundingBox().intersectsWith(aabb) && entity != entityIn) {
                        if (null == p_177414_4_ || p_177414_4_.apply(entity)) {
                        ...
                        if (null != aentity) {
                            for (int l = 0; l < aentity.length; ++l) {
                                ...
                                if (entity != entityIn && entity.getEntityBoundingBox().intersectsWith(aabb) && (null == p_177414_4_ || p_177414_4_.apply(entity))) {
                                    listToFill.add(entity);
                                }
                            }
                        }
    ...
    public <T extends Entity> void getEntitiesOfTypeWithinAAAB(Class<? extends T> entityClass, AxisAlignedBB aabb, List<T> listToFill, Predicate<? super T> p_177430_4_) {
        ...
        for (int k = i; k <= j; ++k) {
            for (T t : this.entityLists[k].getByClass(entityClass)) {
                if (t.getEntityBoundingBox().intersectsWith(aabb) && (null == p_177430_4_ || p_177430_4_.apply(t))) {
                    listToFill.add(t);
                }
            }
        }
    ...
    public void func_150804_b(boolean p_150804_1_) {
        ...
        while (!this.tileEntityPosQueue.isEmpty()) {
            ...
            if (null == getTileEntity(blockpos, EnumCreateEntityType.CHECK) && this.getBlock(blockpos).hasTileEntity()) {
    ...
    public boolean getAreLevelsEmpty(int startY, int endY) {
        ...
        for (int i = startY; i <= endY; i += 16) {
            ...
            if (null != extendedblockstorage && !extendedblockstorage.isEmpty()) {
                return false;
            }
        }
    ...
    public void fillChunk(byte[] p_177439_1_, int p_177439_2_, boolean p_177439_3_) {
        ...
        for (int j = 0; j < this.storageArrays.length; ++j) {
            if (0 != (p_177439_2_ & 1 << j)) {
                if (null == storageArrays[j]) {
            ...
            } else if (p_177439_3_ && null != storageArrays[j]) {
                this.storageArrays[j] = null;
            }
        }

        for (int l = 0; l < this.storageArrays.length; ++l) {
            if (0 != (p_177439_2_ & 1 << l) && null != storageArrays[l]) {
        ...
        if (flag) {
            for (int i1 = 0; i1 < this.storageArrays.length; ++i1) {
                if (0 != (p_177439_2_ & 1 << i1) && null != storageArrays[i1]) {
        ...
        for (int j1 = 0; j1 < this.storageArrays.length; ++j1) {
            if (null != storageArrays[j1] && 0 != (p_177439_2_ & 1 << j1)) {
    ...
    public BiomeGenBase getBiome(BlockPos pos, WorldChunkManager chunkManager) {
        ...
        return null == biomegenbase1 ? BiomeGenBase.plains : biomegenbase1;
    }
    ...
    public void enqueueRelightChecks() {
        ...
        for (int i = 0; 8 > i; ++i) {
            ...
            for (int i1 = 0; 16 > i1; ++i1) {
                ...
                boolean flag = 0 == i1 || 15 == i1 || 0 == k || 15 == k || 0 == l || 15 == l;

                if (null == storageArrays[j] && flag || null != storageArrays[j] && this.storageArrays[j].getBlockByExtId(k, i1, l).getMaterial() == Material.air) {
                    for (EnumFacing enumfacing : EnumFacing.values()) {
                        BlockPos blockpos2 = blockpos1.offset(enumfacing);

                        if (0 < worldObj.getBlockState(blockpos2).getBlock().getLightValue()) {
        ...
        if (!this.worldObj.provider.getHasNoSky()) {
            if (this.worldObj.isAreaLoaded(blockpos.add(-1, 0, -1), blockpos.add(16, this.worldObj.getSeaLevel(), 16))) {
                ...
                if (this.isLightPopulated) {
                    for (EnumFacing enumfacing : EnumFacing.Plane.HORIZONTAL) {
                        int k = EnumFacing.AxisDirection.POSITIVE == enumfacing.getAxisDirection() ? 16 : 1;
    ...
    private void func_180700_a(EnumFacing facing) {
        if (this.isTerrainPopulated) {
            ...
            } else if (EnumFacing.NORTH == facing) {
                for (int l = 0; 16 > l; ++l) {
    ...
    private boolean func_150811_f(int x, int z) {
        ...
        for (int j = i + 16 - 1; j > this.worldObj.getSeaLevel() || 0 < j && !flag1; --j) {
            ...
            if (255 == k && blockpos$mutableblockpos.getY() < this.worldObj.getSeaLevel()) {
                flag1 = true;
            }

            if (!flag && 0 < k) {
                flag = true;
            } else if (flag && 0 == k && !this.worldObj.checkLight(blockpos$mutableblockpos)) {
                return false;
            }
        }

        for (int l = blockpos$mutableblockpos.getY(); 0 < l; --l) {
            blockpos$mutableblockpos.set(blockpos$mutableblockpos.getX(), l, blockpos$mutableblockpos.getZ());

            if (0 < getBlock(blockpos$mutableblockpos).getLightValue()) {
