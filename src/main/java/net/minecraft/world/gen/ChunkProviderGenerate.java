...
public class ChunkProviderGenerate implements IChunkProvider {
    ...
    private void func_147423_a(int x, int y, int z) {
        ...
        for (int k = 0; 5 > k; ++k) {
            for (int l = 0; 5 > l; ++l) {
                ...
                for (int j1 = -i1; i1 >= j1; ++j1) {
                    for (int k1 = -i1; i1 >= k1; ++k1) {
                        ...
                        if (this.field_177475_o == WorldType.AMPLIFIED && 0.0F < f5) {
                            f5 = 1.0F + f5 * 2.0F;
                            f6 = 1.0F + f6 * 4.0F;
                        }
    ...
    public void populate(IChunkProvider chunkProvider, int x, int z) {
        ...
        if (biomegenbase != BiomeGenBase.desert && biomegenbase != BiomeGenBase.desertHills && this.settings.useWaterLakes && !flag && 0 == rand.nextInt(settings.waterLakeChance)) {
        ...
        if (!flag && 0 == rand.nextInt(settings.lavaLakeChance / 10) && this.settings.useLavaLakes) {
            int i2 = this.rand.nextInt(16) + 8;
            int l2 = this.rand.nextInt(this.rand.nextInt(248) + 8);
            int k3 = this.rand.nextInt(16) + 8;

            if (l2 < this.worldObj.getSeaLevel() || 0 == rand.nextInt(settings.lavaLakeChance / 8)) {
    ...
    public boolean populateChunk(IChunkProvider chunkProvider, Chunk chunkIn, int x, int z) {
        ...
        if (this.settings.useMonuments && this.mapFeaturesEnabled && 3600L > chunkIn.getInhabitedTime()) {
    ...
    public List<BiomeGenBase.SpawnListEntry> getPossibleCreatures(EnumCreatureType creatureType, BlockPos pos) {
        ...
        if (this.mapFeaturesEnabled) {
            if (EnumCreatureType.MONSTER == creatureType && this.scatteredFeatureGenerator.func_175798_a(pos)) {
                return this.scatteredFeatureGenerator.getScatteredFeatureSpawnList();
            }

            if (EnumCreatureType.MONSTER == creatureType && this.settings.useMonuments && this.oceanMonumentGenerator.isPositionInStructure(this.worldObj, pos)) {
    ...
    public BlockPos getStrongholdGen(World worldIn, String structureName, BlockPos position) {
        return "Stronghold".equals(structureName) && null != strongholdGenerator ? this.strongholdGenerator.getClosestStrongholdPos(worldIn, position) : null;
    }
