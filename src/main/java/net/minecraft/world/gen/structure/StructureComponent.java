...
public abstract class StructureComponent {
    ...
    public NBTTagCompound createStructureBaseNBT() {
        ...
        nbttagcompound.setInteger("O", null == coordBaseMode ? -1 : this.coordBaseMode.getHorizontalIndex());
    ...
    public void readStructureBaseNBT(World worldIn, NBTTagCompound tagCompound) {
        ...
        this.coordBaseMode = -1 == i ? null : EnumFacing.getHorizontal(i);
    ...
    public static StructureComponent findIntersecting(List<StructureComponent> listIn, StructureBoundingBox boundingboxIn) {
        for (StructureComponent structurecomponent : listIn) {
            if (null != structurecomponent.getBoundingBox() && structurecomponent.getBoundingBox().intersectsWith(boundingboxIn)) {
    ...
    protected int getYWithOffset(int y) {
        return null == coordBaseMode ? y : y + this.boundingBox.minY;
    }
    ...
    protected int getMetadataWithOffset(Block blockIn, int meta) {
        if (blockIn == Blocks.rail) {
            if (EnumFacing.WEST == coordBaseMode || EnumFacing.EAST == coordBaseMode) {
                if (1 == meta) {
                    return 0;
                }
            ...
            if (EnumFacing.SOUTH == coordBaseMode) {
                if (0 == meta) {
                    return 2;
                }
                ...
                if (EnumFacing.WEST == coordBaseMode) {
                    return meta + 1 & 3;
                }

                if (EnumFacing.EAST == coordBaseMode) {
                    return meta + 3 & 3;
                }
            }
            ...
            if (blockIn == Blocks.ladder) {
                ...
                } else if (EnumFacing.WEST == coordBaseMode) {
                    if (meta == EnumFacing.NORTH.getIndex()) {
                ...
                } else if (EnumFacing.EAST == coordBaseMode) {
                    if (meta == EnumFacing.NORTH.getIndex()) {
                        return EnumFacing.EAST.getIndex();
                    }
                ...
                if (EnumFacing.SOUTH == coordBaseMode) {
                    if (EnumFacing.SOUTH == enumfacing || EnumFacing.NORTH == enumfacing) {
        ...
        } else if (EnumFacing.SOUTH == coordBaseMode) {
            if (2 == meta) {
                return 3;
            }
    ...
    protected void clearCurrentPositionBlocksUpwards(World worldIn, int x, int y, int z, StructureBoundingBox structurebb) {
        ...
        if (structurebb.isVecInside(blockpos)) {
            while (!worldIn.isAirBlock(blockpos) && 255 > blockpos.getY()) {
    ...
    protected void replaceAirAndLiquidDownwards(World worldIn, IBlockState blockstateIn, int x, int y, int z, StructureBoundingBox boundingboxIn) {
        ...
        if (boundingboxIn.isVecInside(new BlockPos(i, j, k))) {
            while ((worldIn.isAirBlock(new BlockPos(i, j, k)) || worldIn.getBlockState(new BlockPos(i, j, k)).getBlock().getMaterial().isLiquid()) && 1 < j) {
