...
public class StructureMineshaftPieces {
    ...
    private static StructureComponent func_175890_b(StructureComponent componentIn, List<StructureComponent> listIn, Random rand, int x, int y, int z, EnumFacing facing, int type) {
        if (8 < type) {
            return null;
        } else if (80 >= Math.abs(x - componentIn.getBoundingBox().minX) && 80 >= Math.abs(z - componentIn.getBoundingBox().minZ)) {
    ...
    public static class Corridor extends StructureComponent {
        ...
        public Corridor(int type, Random rand, StructureBoundingBox structurebb, EnumFacing facing) {
            ...
            this.hasRails = 0 == rand.nextInt(3);
            this.hasSpiders = !this.hasRails && 0 == rand.nextInt(23);

            if (EnumFacing.NORTH != coordBaseMode && EnumFacing.SOUTH != coordBaseMode) {
        ...
        public static StructureBoundingBox func_175814_a(List<StructureComponent> p_175814_0_, Random rand, int x, int y, int z, EnumFacing facing) {
            ...
            for (i = rand.nextInt(3) + 2; 0 < i; --i) {
                ...
                if (null == findIntersecting(p_175814_0_, structureboundingbox)) {
                    break;
                }
            }

            return 0 < i ? structureboundingbox : null;
        }
    ...
    public static class Cross extends StructureComponent {
        ...
        public Cross(int type, Random rand, StructureBoundingBox structurebb, EnumFacing facing) {
            ...
            this.boundingBox = structurebb;
            this.isMultipleFloors = 3 < structurebb.getYSize();
        }
        ...
        public static StructureBoundingBox func_175813_a(List<StructureComponent> listIn, Random rand, int x, int y, int z, EnumFacing facing) {
            ...
            return null != findIntersecting(listIn, structureboundingbox) ? null : structureboundingbox;
        }
    ...
    public static class Stairs extends StructureComponent {
        ...
        public boolean addComponentParts(World worldIn, Random randomIn, StructureBoundingBox structureBoundingBoxIn) {
            if (this.isLiquidInStructureBoundingBox(worldIn, structureBoundingBoxIn)) {
                ...
                for (int i = 0; 5 > i; ++i) {
                    this.fillWithBlocks(worldIn, structureBoundingBoxIn, 0, 5 - i - (4 > i ? 1 : 0), 2 + i, 2, 7 - i, 2 + i, Blocks.air.getDefaultState(), Blocks.air.getDefaultState(), false);
                }
