...
public class WalkNodeProcessor extends NodeProcessor {
    ...
    public int findPathOptions(PathPoint[] pathOptions, Entity entityIn, PathPoint currentPoint, PathPoint targetPoint, float maxDistance) {
        ...
        if (1 == getVerticalOffset(entityIn, currentPoint.xCoord, currentPoint.yCoord + 1, currentPoint.zCoord)) {
            j = 1;
        }
        ...
        if (null != pathpoint && !pathpoint.visited && pathpoint.distanceTo(targetPoint) < maxDistance) {
            pathOptions[i++] = pathpoint;
        }

        if (null != pathpoint1 && !pathpoint1.visited && pathpoint1.distanceTo(targetPoint) < maxDistance) {
            pathOptions[i++] = pathpoint1;
        }

        if (null != pathpoint2 && !pathpoint2.visited && pathpoint2.distanceTo(targetPoint) < maxDistance) {
            pathOptions[i++] = pathpoint2;
        }

        if (null != pathpoint3 && !pathpoint3.visited && pathpoint3.distanceTo(targetPoint) < maxDistance) {
    ...
    private PathPoint getSafePoint(Entity entityIn, int x, int y, int z, int p_176171_5_) {
        ...
        if (2 == i) {
            ...
            if (null == pathpoint && 0 < p_176171_5_ && -3 != i && -4 != i && 1 == getVerticalOffset(entityIn, x, y + p_176171_5_, z)) {
            ...
            if (null != pathpoint) {
                ...
                for (k = 0; 0 < y; pathpoint = this.openPoint(x, y, z)) {
                    k = this.getVerticalOffset(entityIn, x, y - 1, z);

                    if (this.avoidsWater && -1 == k) {
                        return null;
                    }
