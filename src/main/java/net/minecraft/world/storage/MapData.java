...
public class MapData extends WorldSavedData {
    ...
    private void updateDecorations(int type, World worldIn, String entityIdentifier, double worldX, double worldZ, double rotation) {
        ...
        if ((float) (-j) <= f && (float) (-j) <= f1 && (float) j >= f && (float) j >= f1) {
            rotation = rotation + (0.0D > rotation ? -8.0D : 8.0D);
            ...
            if (320.0F <= Math.abs(f) || 320.0F <= Math.abs(f1)) {
    ...
    public Packet getMapPacket(ItemStack mapStack, World worldIn, EntityPlayer player) {
        ...
        return null == mapdata$mapinfo ? null : mapdata$mapinfo.getPacket(mapStack);
    }
    ...
    public class MapInfo {
        ...
        public Packet getPacket(ItemStack stack) {
            if (this.field_176105_d) {
                ...
                return 0 == field_176109_i++ % 5 ? new S34PacketMaps(stack.getMetadata(), MapData.this.scale, MapData.this.mapDecorations.values(), MapData.this.colors, 0, 0, 0, 0) : null;
            }
        }
